...
object SparkEnv extends Logging {
  ...
  private def create(
    ...
    val rpcEnv =
      RpcEnv.create(systemName, hostname, port, conf, securityManager, clientMode = !isDriver)
    ...
    val blockTransferService = new NettyBlockTransferService(
        conf, securityManager, hostname, numUsableCores)
  ...
  private[spark] def environmentDetails(
    ...
    val classPathEntries =
      javaClassPath.split(File.pathSeparator).filterNot(_.isEmpty).map((_, "System Classpath"))
