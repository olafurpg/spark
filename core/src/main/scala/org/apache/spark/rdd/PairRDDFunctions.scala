...
class PairRDDFunctions[K, V](self: RDD[(K, V)])(
  ...
  def combineByKeyWithClassTag[C](createCombiner: V => C,
    ...
    val aggregator = new Aggregator[K, V, C](self.context.clean(createCombiner),
  ...
  def combineByKey[C](createCombiner: V => C,
    ...
    combineByKeyWithClassTag(
        createCombiner, mergeValue, mergeCombiners, partitioner, mapSideCombine, serializer)(null)
  }
  ...
  def combineByKeyWithClassTag[C](
    ...
    combineByKeyWithClassTag(
        createCombiner, mergeValue, mergeCombiners, new HashPartitioner(numPartitions))
  }
  ...
  def aggregateByKey[U: ClassTag](zeroValue: U, partitioner: Partitioner)(
    ...
    combineByKeyWithClassTag[U](
        (v: V) => cleanedSeqOp(createZero(), v), cleanedSeqOp, combOp, partitioner)
  }
  ...
  def foldByKey(zeroValue: V, partitioner: Partitioner)(func: (V, V) => V): RDD[(K, V)] =
    self.withScope {
      ...
      combineByKeyWithClassTag[V](
          (v: V) => cleanedFunc(createZero(), v), cleanedFunc, cleanedFunc, partitioner)
    }
  ...
  def countByKeyApprox(
      timeout: Long, confidence: Double = 0.95): PartialResult[Map[K, BoundedDouble]] =
    self.withScope {
  ...
  def countApproxDistinctByKey(p: Int, sp: Int, partitioner: Partitioner): RDD[(K, Long)] =
    self.withScope {
      ...
      combineByKeyWithClassTag(createHLL, mergeValueHLL, mergeHLL, partitioner).mapValues(
          _.cardinality())
    }
   ...
   */
  def fullOuterJoin[W](
      other: RDD[(K, W)], partitioner: Partitioner): RDD[(K, (Option[V], Option[W]))] =
    self.withScope {
   ...
   */
  def combineByKey[C](
      createCombiner: V => C, mergeValue: (C, V) => C, mergeCombiners: (C, C) => C): RDD[(K, C)] =
    self.withScope {
   ...
   */
  def cogroup[W](
      other: RDD[(K, W)], partitioner: Partitioner): RDD[(K, (Iterable[V], Iterable[W]))] =
    self.withScope {
      if (partitioner.isInstanceOf[HashPartitioner] && keyClass.isArray) {
        throw new SparkException("Default partitioner cannot partition array keys.")
      }
  ...
  def cogroup[W1, W2](
      other1: RDD[(K, W1)],
      other2: RDD[(K, W2)]): RDD[(K, (Iterable[V], Iterable[W1], Iterable[W2]))] = self.withScope {
  ...
  def saveAsNewAPIHadoopDataset(conf: Configuration): Unit = self.withScope {
    ...
    val writeShard = (context: TaskContext, iter: Iterator[(K, V)]) => {
      ...
      val attemptId = new TaskAttemptID(
          jobtrackerID, stageId, TaskType.REDUCE, context.partitionId, context.attemptNumber)
