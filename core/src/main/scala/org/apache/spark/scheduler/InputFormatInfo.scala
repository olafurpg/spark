...
class InputFormatInfo(
    val configuration: Configuration, val inputFormatClazz: Class[_], val path: String)
    extends Logging {
  ...
  private def validate() {
    ...
    try {
      ...
      case e: ClassNotFoundException =>
        throw new IllegalArgumentException(
            "Specified inputformat " + inputFormatClazz + " cannot be found ?", e)
    }
  }
  ...
  private def prefLocsFromMapreduceInputFormat(): Set[SplitInfo] = {
    ...
    val instance: org.apache.hadoop.mapreduce.InputFormat[_, _] = ReflectionUtils
