  ...
  def render(request: HttpServletRequest): Seq[Node] = {
    progressListener.synchronized {
      ...
      val accumulableTable =
        UIUtils.listingTable(accumulableHeaders, accumulableRow, externalAccumulables.toSeq)
        ...
        if (validTasks.size == 0) {
          ...
          def getFormattedSizeQuantilesWithRecords(
            ...
            getDistributionQuantiles(data).map(
                d => <td>{s"${Utils.bytesToString(d.toLong)} / ${recordDist.next().toLong}"}</td>)
          }
        ...
        val executorRunTime =
          if (taskInfo.running) {
            ...
            metricsOpt
              ...
              .getOrElse(totalExecutionTime - executorOverhead - gettingResultTime)
          }
...
private[ui] case class TaskTableRowShuffleReadData(shuffleReadBlockedTimeSortable: Long,
...
private[ui] case class TaskTableRowShuffleWriteData(writeTimeSortable: Long,
                                                    writeTimeReadable: String,
                                                    shuffleWriteSortable: Long,
                                                    shuffleWriteReadable: String)

private[ui] case class TaskTableRowBytesSpilledData(memoryBytesSpilledSortable: Long,
 ...
 */
private[ui] class TaskTableRowData(val index: Int,
                                   val taskId: Long,
                                   val attempt: Int,
...
private[ui] class TaskDataSource(tasks: Seq[TaskUIData],
  ...
  private def taskRow(taskData: TaskUIData): TaskTableRowData = {
    ...
    val externalAccumulableReadable = info.accumulables.filterNot(_.internal).flatMap { a =>
    ...
    val inputReadable = maybeInput.map(m => s"${Utils.bytesToString(m.bytesRead)}").getOrElse("")
    ...
    val outputReadable =
      maybeOutput.map(m => s"${Utils.bytesToString(m.bytesWritten)}").getOrElse("")
    ...
    val shuffleWriteReadable =
      maybeShuffleWrite.map(m => s"${Utils.bytesToString(m.bytesWritten)}").getOrElse("")
          ...
          new Ordering[TaskTableRowData] {
            override def compare(x: TaskTableRowData, y: TaskTableRowData): Int =
              Ordering.Long.compare(
                  x.shuffleRead.get.shuffleReadSortable, y.shuffleRead.get.shuffleReadSortable)
          }
        } else {
          throw new IllegalArgumentException(
      ...
      case "Write Time" =>
        if (hasShuffleWrite) {
          new Ordering[TaskTableRowData] {
            override def compare(x: TaskTableRowData, y: TaskTableRowData): Int =
              Ordering.Long.compare(
                  x.shuffleWrite.get.writeTimeSortable, y.shuffleWrite.get.writeTimeSortable)
          }
        } else {
          throw new IllegalArgumentException(
      ...
      case "Shuffle Write Size / Records" =>
        if (hasShuffleWrite) {
          new Ordering[TaskTableRowData] {
            override def compare(x: TaskTableRowData, y: TaskTableRowData): Int =
              Ordering.Long.compare(
                  x.shuffleWrite.get.shuffleWriteSortable, y.shuffleWrite.get.shuffleWriteSortable)
          }
        } else {
          throw new IllegalArgumentException(
...
private[ui] class TaskPagedTable(conf: SparkConf,
  ...
  override def pageLink(page: Int): String = {
    ...
    basePath + s"&$pageNumberFormField=$page" + s"&task.sort=$encodedSortColumn" +
    s"&task.desc=$desc" + s"&$pageSizeFormField=$pageSize"
  }
        ...
        case (header, cssClass) =>
          if (header == sortColumn) {
            ...
            val headerLink = Unparsed(basePath +
                s"&task.sort=${URLEncoder.encode(header, "UTF-8")}" + s"&task.pageSize=$pageSize")
            <th class={cssClass}>
            <a href={headerLink}>
              {header}
            </a>
