...
private object ImplicitOrderingSuite {
  ...
  def basicMapExpectations(rdd: RDD[Int]): List[(Boolean, String)] = {
    List(
        (rdd.map(x => (x, x)).keyOrdering.isDefined, "rdd.map(x => (x, x)).keyOrdering.isDefined"),
        (rdd.map(x => (1, x)).keyOrdering.isDefined, "rdd.map(x => (1, x)).keyOrdering.isDefined"),
  ...
  def otherRDDMethodExpectations(rdd: RDD[Int]): List[(Boolean, String)] = {
    List((rdd.groupBy(x => x).keyOrdering.isDefined, "rdd.groupBy(x => x).keyOrdering.isDefined"),
