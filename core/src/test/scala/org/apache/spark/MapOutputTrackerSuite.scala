...
class MapOutputTrackerSuite extends SparkFunSuite {
  ...
  private def newTrackerMaster(sparkConf: SparkConf = conf) = {
    val broadcastManager = new BroadcastManager(true, sparkConf, new SecurityManager(sparkConf))
  ...
  test("master start and stop") {
    ...
    tracker.trackerEndpoint = rpcEnv.setupEndpoint(
        MapOutputTracker.ENDPOINT_NAME, new MapOutputTrackerMasterEndpoint(rpcEnv, tracker, conf))
    tracker.stop()
    rpcEnv.shutdown()
  }
  ...
  test("master register shuffle and fetch") {
    ...
    tracker.registerMapOutput(
        10, 0, MapStatus(BlockManagerId("a", "hostA", 1000), Array(1000L, 10000L)))
    tracker.registerMapOutput(
        10, 1, MapStatus(BlockManagerId("b", "hostB", 1000), Array(10000L, 1000L)))
  ...
  test("remote fetch below max RPC message size") {
    ...
    masterTracker.registerShuffle(10, 1)
    masterTracker.registerMapOutput(
        10, 0, MapStatus(BlockManagerId("88", "mph", 1000), Array.fill[Long](10)(0)))
...
//    masterTracker.stop() // this throws an exception
  ...
  test("getLocationsWithLargestOutputs with multiple outputs in same machine") {
    ...
    tracker.registerMapOutput(10, 0, MapStatus(BlockManagerId("a", "hostA", 1000), Array(2L)))
    tracker.registerMapOutput(10, 1, MapStatus(BlockManagerId("a", "hostA", 1000), Array(2L)))
    tracker.registerMapOutput(10, 2, MapStatus(BlockManagerId("b", "hostB", 1000), Array(3L)))
  ...
  test("remote fetch using broadcast") {
    ...
    try {
      ...
      (0 until 100).foreach { i =>
        masterTracker.registerMapOutput(20,
                                        i,
