...
abstract class ShuffleSuite extends SparkFunSuite with Matchers with LocalSparkContext {
  ...
  test("sorting on mutable pairs") {
    ...
    val pairs: RDD[MutablePair[Int, Int]] = sc.parallelize(data, 2)
    val results =
      new OrderedRDDFunctions[Int, Int, MutablePair[Int, Int]](pairs).sortByKey().collect()
  ...
  test("metrics for shuffle without aggregation") {
    ...
    val metrics = ShuffleSuite.runAndReturnMetrics(sc) {
      sc.parallelize(1 to numRecords, 4).map(key => (key, 1)).groupByKey().collect()
    }
  ...
  test("metrics for shuffle with aggregation") {
    ...
    val metrics = ShuffleSuite.runAndReturnMetrics(sc) {
      sc.parallelize(1 to numRecords, 4).flatMap(key => Array.fill(100)((key, 1))).countByKey()
    }
  ...
  test("multiple simultaneous attempts for one task (SPARK-8029)") {
    ...
    val interleaver =
      new InterleaveIterators(data1, writeAndClose(writer1), data2, writeAndClose(writer2))
...
object ShuffleSuite {
  ...
  case class AggregatedShuffleMetrics(
      recordsWritten: Long, recordsRead: Long, bytesWritten: Long, byresRead: Long)
