...
class PipedRDDSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("advanced pipe") {
    if (testCommandAvailable("cat")) {
      ...
      val piped = nums.pipe(Seq("cat"), Map[String, String](), (f: String => Unit) => {
        bl.value.map(f(_)); f("\u0001")
  ...
  def testExportInputFile(varName: String) {
    if (testCommandAvailable("printenv")) {
      val nums = new HadoopRDD(
          sc, new JobConf(), classOf[TextInputFormat], classOf[LongWritable], classOf[Text], 2) {
  ...
  def generateFakeHadoopPartition(): HadoopPartition = {
    val split = new FileSplit(
        new Path("/some/path"), 0, 1, Array[String]("loc1", "loc2", "loc3", "loc4", "loc5"))
