...
class RDDSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("coalesced RDDs") {
    ...
    val isEquals =
      coalesced5.dependencies.head.rdd.dependencies.head.rdd.asInstanceOf[ShuffledRDD[_, _, _]] != null
    assert(isEquals)
  ...
  test("coalesced RDDs with locality, large scale (10K partitions)") {
    ...
    for (seed <- 1 to 5) {
      ...
      assert(minLocality2 >= 0.90,
             "Expected 90% locality for derived RDD but got " + (minLocality2 * 100.0).toInt + "%")
    }
  }
  ...
  test("coalesced RDDs with partial locality, large scale (10K partitions)") {
    ...
    for (seed <- 1 to 5) {
      ...
      assert(coalesced2.partitions.size == 10000,
             "Expected 10000 partitions, but got " + coalesced2.partitions.size)

      // test that we have 100 partitions
      val coalesced3 = data2.coalesce(numMachines * 2)
      assert(coalesced3.partitions.size == 100,
             "Expected 100 partitions, but got " + coalesced3.partitions.size)
