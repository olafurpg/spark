...
class TaskSchedulerImplSuite extends SparkFunSuite with LocalSparkContext with Logging {
  ...
  test("Scheduler correctly accounts for multiple CPUs per task") {
    ...
    val zeroCoreWorkerOffers =
      Seq(new WorkerOffer("executor0", "host0", 0), new WorkerOffer("executor1", "host1", 0))
    val taskSet = FakeTask.createTaskSet(1)
    taskScheduler.submitTasks(taskSet)
  ...
  test("refuse to schedule concurrent attempts for the same stage (SPARK-8103)") {
    ...
    taskScheduler.taskSetManagerForAttempt(attempt1.stageId, attempt1.stageAttemptId).get.isZombie =
      true
    taskScheduler.submitTasks(attempt2)
    val attempt3 = FakeTask.createTaskSet(1, 2)
    intercept[IllegalStateException] { taskScheduler.submitTasks(attempt3) }
    taskScheduler.taskSetManagerForAttempt(attempt2.stageId, attempt2.stageAttemptId).get.isZombie =
      true
