...
class UISeleniumSuite extends SparkFunSuite with WebBrowser with Matchers with BeforeAndAfterAll {
  ...
  test("live UI json application list") {
    withSpark(newSparkContext()) { sc =>
      val appListRawJson =
        HistoryServerSuite.getUrl(new URL(sc.ui.get.appUIAddress + "/api/v1/applications"))
  ...
  test("job stages should have expected dotfile under DAG visualization") {
    withSpark(newSparkContext()) { sc =>
      ...
      val stage0 = Source
        .fromURL(sc.ui.get.appUIAddress + "/stages/stage/?id=0&attempt=0&expandDagViz=true")
        .mkString
      ...
      val stage1 = Source
        .fromURL(sc.ui.get.appUIAddress + "/stages/stage/?id=1&attempt=0&expandDagViz=true")
        .mkString
      ...
      val stage2 = Source
        .fromURL(sc.ui.get.appUIAddress + "/stages/stage/?id=2&attempt=0&expandDagViz=true")
        .mkString
