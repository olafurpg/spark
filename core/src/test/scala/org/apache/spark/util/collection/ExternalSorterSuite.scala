...
class ExternalSorterSuite extends SparkFunSuite with LocalSparkContext {
  ...
  testWithMultipleSer("spilling in local cluster", loadDefaults = true) { (conf: SparkConf) =>
  ...
  test("spilling with hash collisions") {
    ...
    def mergeCombiners(
        buffer1: ArrayBuffer[String], buffer2: ArrayBuffer[String]): ArrayBuffer[String] =
      buffer1 ++= buffer2
    ...
    val sorter =
      new ExternalSorter[String, String, ArrayBuffer[String]](context, Some(agg), None, None)
    ...
    val toInsert =
      (1 to size).iterator.map(_.toString).map(s => (s, s)) ++
  ...
  test("spilling with hash collisions using the Int.MaxValue key") {
    ...
    val agg =
      new Aggregator[Int, Int, ArrayBuffer[Int]](createCombiner, mergeValue, mergeCombiners)
    val sorter = new ExternalSorter[Int, Int, ArrayBuffer[Int]](context, Some(agg), None, None)
    sorter.insertAll(
  ...
  private def emptyDataStream(conf: SparkConf) {
    ...
    // Only aggregator
    val sorter2 =
      new ExternalSorter[Int, Int, Int](context, Some(agg), Some(new HashPartitioner(3)), None)
    assert(sorter2.iterator.toSeq === Seq())
    sorter2.stop()

    // Only ordering
    val sorter3 =
      new ExternalSorter[Int, Int, Int](context, None, Some(new HashPartitioner(3)), Some(ord))
    assert(sorter3.iterator.toSeq === Seq())
    sorter3.stop()

    // Neither aggregator nor ordering
    val sorter4 =
      new ExternalSorter[Int, Int, Int](context, None, Some(new HashPartitioner(3)), None)
  ...
  private def fewElementsPerPartition(conf: SparkConf) {
    ...
    // Only aggregator
    val sorter2 =
      new ExternalSorter[Int, Int, Int](context, Some(agg), Some(new HashPartitioner(7)), None)
    ...
    // Only ordering
    val sorter3 =
      new ExternalSorter[Int, Int, Int](context, None, Some(new HashPartitioner(7)), Some(ord))
    ...
    // Neither aggregator nor ordering
    val sorter4 =
      new ExternalSorter[Int, Int, Int](context, None, Some(new HashPartitioner(7)), None)
  ...
  private def basicSorterTest(conf: SparkConf,
    ...
    val context = MemoryTestingUtils.fakeTaskContext(sc.env)
    val sorter = new ExternalSorter[Int, Int, Int](context, agg, Some(new HashPartitioner(3)), ord)
    sorter.insertAll(
        (0 until size).iterator.map { i =>
      (i / 4, i)
    })
  ...
  private def sortWithoutBreakingSortingContracts(conf: SparkConf) {
    ...
    val context = MemoryTestingUtils.fakeTaskContext(sc.env)
    val sorter1 =
      new ExternalSorter[String, String, String](context, None, None, Some(wrongOrdering))
