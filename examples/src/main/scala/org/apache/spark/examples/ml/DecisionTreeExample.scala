...
object DecisionTreeExample {
  ...
  private[ml] def loadData(spark: SparkSession,
                           path: String,
                           format: String,
      ...
      case "libsvm" =>
        expectedNumFeatures match {
          case Some(numFeatures) =>
            spark.read.option("numFeatures", numFeatures.toString).format("libsvm").load(path)
  ...
  private[ml] def loadDatasets(input: String,
    ...
    val dataframes: Array[DataFrame] =
      if (testInput != "") {
        ...
        val origTestExamples: DataFrame = loadData(spark, testInput, dataFormat, Some(numFeatures))
  ...
  def run(params: Params) {
    val spark = SparkSession.builder.appName(s"DecisionTreeExample with $params").getOrCreate()
