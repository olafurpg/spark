    ...
    def getBlockFromKinesis(): Iterator[T] = {
      ...
      partition.seqNumberRanges.ranges.iterator.flatMap { range =>
        new KinesisSequenceRangeIterator(
            credentials, endpointUrl, regionName, range, retryTimeoutMs).map(messageHandler)
      }
    }
 ...
 */
private[kinesis] class KinesisSequenceRangeIterator(credentials: AWSCredentials,
                                                    endpointUrl: String,
                                                    regionId: String,
  ...
  private def getRecordsAndNextKinesisIterator(shardIterator: String): (Iterator[Record], String) = {
    ...
    val getRecordsResult =
      retryOrTimeout[GetRecordsResult](s"getting records using shard iterator") {
   ...
   * sequence number.
   */
  private def getKinesisIterator(iteratorType: ShardIteratorType, sequenceNumber: String): String = {
