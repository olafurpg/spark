    ...
    while (sccWorkGraph.numVertices > 0 && iter < numIter) {
      ...
      do {
        ...
        sccWorkGraph = sccWorkGraph
          .outerJoinVertices(sccWorkGraph.outDegrees) { (vid, data, degreeOpt) =>
        ...
        val finalVertices = sccWorkGraph.vertices.filter { case (vid, (scc, isFinal)) => isFinal }.mapValues {
          (vid, data) =>
            data._1
        }

        // write values to sccGraph
        sccGraph = sccGraph.outerJoinVertices(finalVertices) { (vid, scc, opt) =>
          opt.getOrElse(scc)
        }
      ...
      sccWorkGraph = Pregel[(VertexId, Boolean), ED, VertexId](
          ...
          (vid, myScc, neighborScc) => (math.min(myScc._1, neighborScc), myScc._2), e => {
