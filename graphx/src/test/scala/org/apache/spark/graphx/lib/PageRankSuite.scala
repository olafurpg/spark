...
class PageRankSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("Star PersonalPageRank") {
    withSpark { sc =>
      ...
      val staticRanks2 =
        starGraph.staticPersonalizedPageRank(0, numIter = 2, resetProb).vertices.cache()
      ...
      val otherStaticRanks2 =
        starGraph.staticPersonalizedPageRank(1, numIter = 2, resetProb).vertices.cache()
  ...
  test("Grid PageRank") {
    withSpark { sc =>
      ...
      val referenceRanks =
        VertexRDD(sc.parallelize(GridPageRank(rows, cols, numIter, resetProb))).cache()
