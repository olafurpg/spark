...
object NaiveBayesModel extends MLReadable[NaiveBayesModel] {
  ...
  private[ml] def fromOld(oldModel: OldNaiveBayesModel, parent: NaiveBayes): NaiveBayesModel = {
    ...
    val theta = new DenseMatrix(
        oldModel.labels.length, oldModel.theta(0).length, oldModel.theta.flatten, true)
  ...
  private[NaiveBayesModel] class NaiveBayesModelWriter(instance: NaiveBayesModel)
      extends MLWriter {
