...
private[clustering] trait LDAParams
  ...
  private[clustering] def getOldOptimizer: OldLDAOptimizer = getOptimizer match {
    ...
    case "em" =>
      new OldEMLDAOptimizer().setKeepLastCheckpoint($(keepLastCheckpoint))
  }
}
  ...
  private class LocalLDAModelReader extends MLReader[LocalLDAModel] {
    ...
    override def load(path: String): LocalLDAModel = {
      ...
      val data = sparkSession.read
        .parquet(dataPath)
        .select(
            "vocabSize", "topicsMatrix", "docConcentration", "topicConcentration", "gammaShape")
        .head()
...
@Since("1.6.0")
@Experimental
class DistributedLDAModel private[ml](uid: String,
                                      vocabSize: Int,
...
private[clustering] object LDA extends DefaultParamsReadable[LDA] {
  ...
  def getOldDataset(dataset: Dataset[_], featuresCol: String): RDD[(Long, Vector)] = {
    dataset.withColumn("docId", monotonicallyIncreasingId()).select("docId", featuresCol).rdd.map {
