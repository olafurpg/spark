...
private[feature] trait CountVectorizerParams extends Params with HasInputCol with HasOutputCol {
  ...
  val binary: BooleanParam = new BooleanParam(
      this, "binary", "If True, all non zero counts are set to 1.")
...
class CountVectorizer(override val uid: String)
  ...
  override def fit(dataset: Dataset[_]): CountVectorizerModel = {
    ...
    val vocab = wordCounts.top(math.min(fullVocabSize, vocSize).toInt)(Ordering.by(_._2)).map(_._1)
...
object CountVectorizerModel extends MLReadable[CountVectorizerModel] {
  ...
  private[CountVectorizerModel] class CountVectorizerModelWriter(instance: CountVectorizerModel)
      extends MLWriter {
