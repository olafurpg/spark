...
class OneHotEncoder(override val uid: String)
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    val outputAttrNames: Option[Array[String]] = inputAttr match {
      ...
      case _: NumericAttribute =>
        throw new RuntimeException(s"The input column $inputColName cannot be numeric.")
      case _ =>
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    var outputAttrGroup =
      AttributeGroup.fromStructField(transformSchema(dataset.schema)(outputColName))
    ...
    dataset.select(
        col("*"), encode(col(inputColName).cast(DoubleType)).as(outputColName, metadata))
  }
