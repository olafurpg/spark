...
private[spark] object GradientBoostedTrees extends Logging {
    ...
    algo match {
      ...
      case OldAlgo.Classification =>
        ...
        GradientBoostedTrees.boost(
            remappedInput, remappedInput, boostingStrategy, validate = false, seed)
      case _ =>
   ...
   *         (array of decision tree models, array of model weights)
   */
  def runWithValidation(input: RDD[LabeledPoint],
    ...
    algo match {
      ...
      case OldAlgo.Classification =>
        ...
        val remappedValidationInput =
          validationInput.map(x => new LabeledPoint((x.label * 2) - 1, x.features))
        GradientBoostedTrees.boost(
            remappedInput, remappedValidationInput, boostingStrategy, validate = true, seed)
      case _ =>
   ...
   */
  def updatePrediction(features: Vector,
                       prediction: Double,
                       tree: DecisionTreeRegressionModel,
   ...
   */
  def computeError(data: RDD[LabeledPoint],
                   trees: Array[DecisionTreeRegressionModel],
                   treeWeights: Array[Double],
                   loss: OldLoss): Double = {
    data.map { lp =>
      val predicted = trees.zip(treeWeights).foldLeft(0.0) {
        case (acc, (model, weight)) =>
  ...
  def boost(input: RDD[LabeledPoint],
    ...
    var validatePredError: RDD[(Double, Double)] = computeInitialPredictionAndError(
    ...
    while (m < numIterations && !doneLearning) {
      ...
      if (validate) {
        ...
        if (bestValidateError -
            currentValidateError < validationTol * Math.max(currentValidateError, 0.01)) {
          doneLearning = true
