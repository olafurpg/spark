...
object RandomRDDs {
  ...
  @Since("1.1.0")
  def uniformJavaRDD(
      jsc: JavaSparkContext, size: Long, numPartitions: Int, seed: Long): JavaDoubleRDD = {
  ...
  @Since("1.1.0")
  def poissonJavaRDD(
      jsc: JavaSparkContext, mean: Double, size: Long, numPartitions: Int): JavaDoubleRDD = {
  ...
  @Since("1.3.0")
  def gammaJavaRDD(
      jsc: JavaSparkContext, shape: Double, scale: Double, size: Long): JavaDoubleRDD = {
  ...
  def logNormalJavaRDD(
      jsc: JavaSparkContext, mean: Double, std: Double, size: Long): JavaDoubleRDD = {
  ...
  @Since("1.6.0")
  def randomJavaRDD[T](
      jsc: JavaSparkContext, generator: RandomDataGenerator[T], size: Long): JavaRDD[T] = {
  ...
  def uniformJavaVectorRDD(
      jsc: JavaSparkContext, numRows: Long, numCols: Int, numPartitions: Int): JavaRDD[Vector] = {
  ...
  def logNormalVectorRDD(sc: SparkContext,
    ...
    val logNormal = new LogNormalGenerator(mean, std)
    randomVectorRDD(
        sc, logNormal, numRows, numCols, numPartitionsOrDefault(sc, numPartitions), seed)
  }
  ...
  def poissonJavaVectorRDD(
      jsc: JavaSparkContext, mean: Double, numRows: Long, numCols: Int): JavaRDD[Vector] = {
   ...
   */
  @Since("1.3.0")
  def exponentialVectorRDD(sc: SparkContext,
                           mean: Double,
                           numRows: Long,
                           numCols: Int,
                           numPartitions: Int = 0,
    ...
    val exponential = new ExponentialGenerator(mean)
    randomVectorRDD(
        sc, exponential, numRows, numCols, numPartitionsOrDefault(sc, numPartitions), seed)
  }
