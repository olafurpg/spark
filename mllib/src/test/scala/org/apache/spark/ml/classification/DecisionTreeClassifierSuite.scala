...
class DecisionTreeClassifierSuite
  ...
  test("Binary classification stump with ordered categorical features") {
    val dt =
      new DecisionTreeClassifier().setImpurity("gini").setMaxDepth(2).setMaxBins(100).setSeed(1)
  ...
  test("Multiclass classification stump with unordered categorical features," +
    ...
    val rdd = categoricalDataPointsForMulticlassRDD
    val dt = new DecisionTreeClassifier().setImpurity("Gini").setMaxDepth(4).setMaxBins(maxBins)
    val categoricalFeatures = Map(0 -> 3, 1 -> 3)
    val numClasses = 3
    compareAPIs(rdd, dt, categoricalFeatures, numClasses)
  }
  ...
  test("Multiclass classification stump with continuous features") {
    ...
    val dt = new DecisionTreeClassifier().setImpurity("Gini").setMaxDepth(4).setMaxBins(100)
    val numClasses = 3
    compareAPIs(rdd, dt, categoricalFeatures = Map.empty[Int, Int], numClasses)
  }
  ...
  test("Multiclass classification tree with 10-ary (ordered) categorical features," +
      " with just enough bins") {
    val rdd = categoricalDataPointsForMulticlassForOrderedFeaturesRDD
    val dt = new DecisionTreeClassifier().setImpurity("Gini").setMaxDepth(4).setMaxBins(10)
    val categoricalFeatures = Map(0 -> 10, 1 -> 10)
    val numClasses = 3
    compareAPIs(rdd, dt, categoricalFeatures, numClasses)
  }
  ...
  test("split must satisfy min instances per node requirements") {
    ...
    val rdd = sc.parallelize(arr)
    val dt =
      new DecisionTreeClassifier().setImpurity("Gini").setMaxDepth(2).setMinInstancesPerNode(2)
    val numClasses = 2
    compareAPIs(rdd, dt, categoricalFeatures = Map.empty[Int, Int], numClasses)
  }
  ...
  test("split must satisfy min info gain requirements") {
    ...
    val dt = new DecisionTreeClassifier().setImpurity("Gini").setMaxDepth(2).setMinInfoGain(1.0)
    val numClasses = 2
    compareAPIs(rdd, dt, categoricalFeatures = Map.empty[Int, Int], numClasses)
  }
  ...
  test("Use soft prediction for binary classification with ordered categorical features") {
    ...
    val dt = new DecisionTreeClassifier().setImpurity("gini").setMaxDepth(1).setMaxBins(3)
    val model = dt.fit(df)
  ...
  test("Feature importance with toy data") {
    val dt = new DecisionTreeClassifier().setImpurity("gini").setMaxDepth(3).setSeed(123)
  ...
  test("read/write") {
    def checkModelData(
        model: DecisionTreeClassificationModel, model2: DecisionTreeClassificationModel): Unit = {
    ...
    testEstimatorAndModelReadWrite(
        dt, continuousData, allParamSettings ++ Map("maxDepth" -> 0), checkModelData)
  }
}
