...
class CountVectorizerSuite
    ...
    val df = spark
      .createDataFrame(
          Seq(
              ...
              (4, split("a notInDict d"), Vectors.sparse(4, Seq((0, 1.0), (3, 1.0)))) // with words not in vocabulary
          ))
  ...
  test("CountVectorizer vocabSize and minDF") {
    val df = spark
      .createDataFrame(Seq((0, split("a b c d"), Vectors.sparse(3, Seq((0, 1.0), (1, 1.0)))),
    ...
    val cvModel2 =
      new CountVectorizer().setInputCol("words").setOutputCol("features").setMinDF(3).fit(df)
  ...
  test("CountVectorizerModel with minTF count") {
    val df = spark
      .createDataFrame(
          Seq((0, split("a a a b b c c c d "), Vectors.sparse(4, Seq((0, 3.0), (2, 3.0)))),
  ...
  test("CountVectorizerModel and CountVectorizer with binary") {
    ...
    val cv =
      new CountVectorizer().setInputCol("words").setOutputCol("features").setBinary(true).fit(df)
    cv.transform(df).select("features", "expected").collect().foreach {
      case Row(features: Vector, expected: Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
