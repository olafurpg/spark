...
class MaxAbsScalerSuite
  ...
  test("MaxAbsScaler fit basic case") {
    ...
    val expected: Array[Vector] = Array(Vectors.dense(0.5, 0, 1),
                                        Vectors.dense(1, 0, 0),
    ...
    val scaler = new MaxAbsScaler().setInputCol("features").setOutputCol("scaled")
  ...
  test("MaxAbsScaler read/write") {
    val t = new MaxAbsScaler().setInputCol("myInputCol").setOutputCol("myOutputCol")
    testDefaultReadWrite(t)
  }

  test("MaxAbsScalerModel read/write") {
    val instance = new MaxAbsScalerModel("myMaxAbsScalerModel", Vectors.dense(1.0, 10.0))
