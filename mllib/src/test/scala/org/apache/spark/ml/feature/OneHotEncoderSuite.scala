...
class OneHotEncoderSuite
  ...
  test("OneHotEncoder dropLast = false") {
    ...
    val encoder =
      new OneHotEncoder().setInputCol("labelIndex").setOutputCol("labelVec").setDropLast(false)
  ...
  test("OneHotEncoder dropLast = true") {
    ...
    // a -> 0, b -> 2, c -> 1
    val expected =
      Set((0, 1.0, 0.0), (1, 0.0, 0.0), (2, 0.0, 1.0), (3, 1.0, 0.0), (4, 1.0, 0.0), (5, 0.0, 1.0))
    assert(output === expected)
  }
  ...
  test("read/write") {
    val t =
      new OneHotEncoder().setInputCol("myInputCol").setOutputCol("myOutputCol").setDropLast(false)
  ...
  test("OneHotEncoder with varying types") {
    ...
    val cols =
      Array("labelIndex", "shortLabel", "longLabel", "intLabel", "floatLabel", "decimalLabel")
    for (col <- cols) {
      val encoder =
        new OneHotEncoder().setInputCol(col).setOutputCol("labelVec").setDropLast(false)
