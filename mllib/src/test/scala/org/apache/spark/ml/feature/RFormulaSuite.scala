...
class RFormulaSuite extends SparkFunSuite with MLlibTestSparkContext with DefaultReadWriteTest {
  ...
  test("transform numeric data") {
    ...
    val original = spark.createDataFrame(Seq((0, 1.0, 3.0), (2, 2.0, 5.0))).toDF("id", "v1", "v2")
  ...
  test("allow empty label") {
    ...
    val expected = spark
      .createDataFrame(
          Seq((1, 2.0, 3.0, Vectors.dense(2.0, 3.0)),
  ...
  test("attribute generation") {
    ...
    val expectedAttrs = new AttributeGroup("features",
  ...
  test("vector attribute generation with unnamed input attrs") {
    ...
    val metadata = new AttributeGroup(
        "vec2",
        Array[Attribute](NumericAttribute.defaultAttr, NumericAttribute.defaultAttr)).toMetadata()
  ...
  test("numeric interaction") {
    ...
    val expected = spark
      .createDataFrame(
          Seq((1, 2, 4, 2, Vectors.dense(16.0), 1.0), (2, 3, 4, 1, Vectors.dense(12.0), 2.0))
      )
      .toDF("a", "b", "c", "d", "features", "label")
    assert(result.collect() === expected.collect())
    val attrs = AttributeGroup.fromStructField(result.schema("features"))
    val expectedAttrs = new AttributeGroup(
        "features", Array[Attribute](new NumericAttribute(Some("b:c:d"), Some(1))))
    assert(attrs === expectedAttrs)
  }
  ...
  test("read/write: RFormula") {
    val rFormula =
      new RFormula().setFormula("id ~ a:b").setFeaturesCol("myFeatures").setLabelCol("myLabels")
  ...
  test("should support all NumericType labels") {
    val formula =
      new RFormula().setFormula("label ~ features").setLabelCol("x").setFeaturesCol("y")
