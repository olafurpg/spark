...
class StopWordsRemoverSuite
  ...
  test("StopWordsRemover with particular stop words list") {
    ...
    val remover =
      new StopWordsRemover().setInputCol("raw").setOutputCol("filtered").setStopWords(stopWords)
    val dataSet = spark
      .createDataFrame(
          Seq(
              (Seq("test", "test"), Seq()),
  ...
  test("StopWordsRemover case sensitive") {
    val remover =
      new StopWordsRemover().setInputCol("raw").setOutputCol("filtered").setCaseSensitive(true)
    val dataSet = spark
      .createDataFrame(Seq(
              (Seq("A"), Seq("A")),
              (Seq("The", "the"), Seq("The"))
  ...
  test("StopWordsRemover with additional words") {
    val stopWords =
      StopWordsRemover.loadDefaultStopWords("english").toSet ++ Set("python", "scala")
    val remover = new StopWordsRemover()
      .setInputCol("raw")
      .setOutputCol("filtered")
      .setStopWords(stopWords.toArray)
  ...
  test("StopWordsRemover output column already exists") {
    ...
    val remover = new StopWordsRemover().setInputCol("raw").setOutputCol(outputCol)
    val dataSet = spark
      .createDataFrame(Seq(
