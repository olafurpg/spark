...
class VectorAssemblerSuite
  ...
  test("VectorAssembler") {
    ...
    val assembler =
      new VectorAssembler().setInputCols(Array("x", "y", "z", "n")).setOutputCol("features")
  ...
  test("transform should throw an exception in case of unsupported type") {
    ...
    val assembler =
      new VectorAssembler().setInputCols(Array("a", "b", "c")).setOutputCol("features")
  ...
  test("ML attributes") {
    ...
    val user = new AttributeGroup(
        "user",
        Array(NominalAttribute.defaultAttr.withName("gender").withValues("male", "female"),
    ...
    val df = spark
      ...
      .select(col("browser").as("browser", browser.toMetadata()),
