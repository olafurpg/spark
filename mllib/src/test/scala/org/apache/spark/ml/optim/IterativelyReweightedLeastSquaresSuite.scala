...
class IterativelyReweightedLeastSquaresSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("IRLS against GLM with Binomial errors") {
    ...
    for (fitIntercept <- Seq(false, true)) {
      ...
      val irls = new IterativelyReweightedLeastSquares(
          initial, BinomialReweightFunc, fitIntercept, regParam = 0.0, maxIter = 25, tol = 1e-8)
        .fit(instances1)
  ...
  test("IRLS against GLM with Poisson errors") {
     ...
     */
    val expected = Seq(
        Vectors.dense(0.0, -0.09607792, 0.18375613), Vectors.dense(6.299947, 3.324107, -1.081766))
    ...
    for (fitIntercept <- Seq(false, true)) {
      ...
      val initial = new WeightedLeastSquares(fitIntercept,
                                             ...
                                             standardizeLabel = false).fit(newInstances)
      val irls = new IterativelyReweightedLeastSquares(
          initial, PoissonReweightFunc, fitIntercept, regParam = 0.0, maxIter = 25, tol = 1e-8)
        .fit(instances2)
      val actual = Vectors.dense(irls.intercept, irls.coefficients(0), irls.coefficients(1))
      assert(actual ~== expected(idx) absTol 1e-4)
      idx += 1
  ...
  test("IRLS against L1Regression") {
    /*
       ...
       [1] 29.5 17.0 -5.5
     */
    val expected = Seq(Vectors.dense(0.0, 1.266667, 0.400000), Vectors.dense(29.5, 17.0, -5.5))
...
object IterativelyReweightedLeastSquaresSuite {
  ...
  def BinomialReweightFunc(
      instance: Instance, model: WeightedLeastSquaresModel): (Double, Double) = {
  ...
  def PoissonReweightFunc(instance: Instance, model: WeightedLeastSquaresModel): (Double, Double) = {
    val eta = model.predict(instance.features)
    val mu = math.exp(eta)
