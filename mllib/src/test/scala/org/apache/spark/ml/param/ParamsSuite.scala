...
class ParamsSuite extends SparkFunSuite {
  ...
  test("json encode/decode") {
...

    {
      ...
      val values: Seq[Array[Int]] = Seq(Array(), Array(1), Array(Int.MinValue, 0, Int.MaxValue))
      for (value <- values) {
        val json = param.jsonEncode(value)
        assert(param.jsonDecode(json) === value)
      }
  ...
  test("ParamValidate") {
    ...
    assert(inRange02DoubleInclusive(0) && inRange02DoubleInclusive(1) && inRange02DoubleInclusive(
            2) && !inRange02DoubleInclusive(-0.1) && !inRange02DoubleInclusive(2.1))
