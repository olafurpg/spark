...
class GeneralizedLinearRegressionSuite
  ...
  override def beforeAll(): Unit = {
    ...
    datasetGaussianIdentity = spark.createDataFrame(
        sc.parallelize(generateGeneralizedLinearRegressionInput(intercept = 2.5,
    ...
    datasetGaussianLog = spark.createDataFrame(
        sc.parallelize(generateGeneralizedLinearRegressionInput(intercept = 0.25,
  ...
  ignore("export test data into CSV format") {
    datasetGaussianIdentity.rdd.map {
      ...
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetGaussianIdentity")
    datasetGaussianLog.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetGaussianLog")
    datasetGaussianInverse.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetGaussianInverse")
    datasetBinomial.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1).saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetBinomial")
    datasetPoissonLog.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetPoissonLog")
    datasetPoissonIdentity.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetPoissonIdentity")
    datasetPoissonSqrt.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetPoissonSqrt")
    datasetGammaInverse.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetGammaInverse")
    datasetGammaIdentity.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1)
      .saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetGammaIdentity")
    datasetGammaLog.rdd.map {
      case Row(label: Double, features: Vector) =>
        label + "," + features.toArray.mkString(",")
    }.repartition(1).saveAsTextFile("target/tmp/GeneralizedLinearRegressionSuite/datasetGammaLog")
  }
  ...
  test("generalized linear regression: default params") {
    ...
    val model = glr.setFamily("gaussian").setLink("identity").fit(datasetGaussianIdentity)
  ...
  test("generalized linear regression: gaussian family against glm") {
    ...
    for ((link, dataset) <- Seq(("identity", datasetGaussianIdentity),
      ...
      for (fitIntercept <- Seq(false, true)) {
        ...
        model
          .transform(dataset)
          .select("features", "prediction", "linkPrediction")
          .collect()
          .foreach {
            case Row(features: DenseVector, prediction1: Double, linkPrediction1: Double) =>
              ...
              assert(linkPrediction1 ~= linkPrediction2 relTol 1E-5,
                     "Link Prediction mismatch: " +
  ...
  test("generalized linear regression: binomial family against glm") {
     ...
     */
    val expected = Seq(Vectors.dense(0.0, -0.3560284, 1.3010002, -0.3570805, -0.7406762),
  ...
  test("glm summary: gaussian family with weight") {
     ...
     */
    val datasetWithWeight = spark.createDataFrame(
        sc.parallelize(Seq(
    ...
    val devianceResiduals =
      summary.residuals().select(col("devianceResiduals")).collect().map(_.getDouble(0))
    val pearsonResiduals =
      summary.residuals("pearson").select(col("pearsonResiduals")).collect().map(_.getDouble(0))
    val workingResiduals =
      summary.residuals("working").select(col("workingResiduals")).collect().map(_.getDouble(0))
    val responseResiduals =
      summary.residuals("response").select(col("responseResiduals")).collect().map(_.getDouble(0))
  ...
  test("glm summary: gamma family with weight") {
     ...
     */
    val trainer = new GeneralizedLinearRegression().setFamily("gamma").setWeightCol("weight")
  ...
  test("read/write") {
    ...
    testEstimatorAndModelReadWrite(
        glr, datasetPoissonLog, GeneralizedLinearRegressionSuite.allParamSettings, checkModelData)
  }

  test("should support all NumericType labels and not support other types") {
    val glr = new GeneralizedLinearRegression().setMaxIter(1)
    MLTestingUtils
      .checkNumericTypes[GeneralizedLinearRegressionModel, GeneralizedLinearRegression](
...
object GeneralizedLinearRegressionSuite {
   ...
   */
  val allParamSettings: Map[String, Any] = Map("family" -> "poisson",
                                               "link" -> "log",
                                               "fitIntercept" -> true,
  ...
  def generateGeneralizedLinearRegressionInput(intercept: Double,
