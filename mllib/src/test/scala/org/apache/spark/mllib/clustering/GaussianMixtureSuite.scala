...
class GaussianMixtureSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("two clusters") {
    ...
    val gmm = new GaussianMixture().setK(2).setInitialModel(initialGmm).run(data)
  ...
  test("single cluster with sparse data") {
    ...
    val Esigma = Matrices.dense(
        3, 3, Array(8.0 / 3.0, -4.0, 4.0 / 3.0, -4.0, 8.0, -4.0, 4.0 / 3.0, -4.0, 8.0 / 3.0))
  ...
  test("two clusters with sparse data") {
    ...
    val sparseGMM = new GaussianMixture().setK(2).setInitialModel(initialGmm).run(sparseData)
