...
class HashingTFSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("hashing tf on a single doc") {
    ...
    assert(
        termFreqs.map(_._1).forall(i => i >= 0 && i < n), "index must be in range [0, #features)")
  ...
  test("hashing tf on an RDD") {
    ...
    val localDocs: Seq[Seq[String]] =
      Seq("a a b b b c d".split(" "), "a b c d a b c".split(" "), "c b a c b a a".split(" "))
