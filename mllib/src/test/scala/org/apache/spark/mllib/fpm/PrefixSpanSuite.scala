...
class PrefixSpanSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("PrefixSpan internal (integer seq, 0 delim) run, singleton itemsets") {
    ...
    val result3 =
      PrefixSpan.genFreqPatterns(rdd, minCount = 2, maxPatternLength = 2, maxLocalProjDBSize = 32L)
  ...
  test("model save/load") {
    ...
    try {
      ...
      val originalSet = model.freqSequences
        .collect()
        .map { x =>
          (x.sequence.map(_.toSet).toSeq, x.freq)
        }
        .toSet
      val newSet = newModel.freqSequences
        .collect()
        .map { x =>
          (x.sequence.map(_.toSet).toSeq, x.freq)
        }
  ...
  private def compareResults[Item](expectedValue: Array[(Array[Array[Item]], Long)],
  ...
  private def compareInternalResults(
      expectedValue: Array[(Array[Int], Long)], actualValue: Array[(Array[Int], Long)]): Unit = {
