...
object GradientDescentSuite {
  ...
  def generateGDInput(offset: Double, scale: Double, nPoints: Int, seed: Int): Seq[LabeledPoint] = {
...
class GradientDescentSuite extends SparkFunSuite with MLlibTestSparkContext with Matchers {
  ...
  test("Assert the loss is decreasing.") {
    ...
    val (_, loss) = GradientDescent.runMiniBatchSGD(dataRDD,
                                                    gradient,
                                                    updater,
                                                    stepSize,
                                                    numIterations,
  ...
  test("Test the loss and gradient of first iteration with regularization.") {
    ...
    assert(loss1(0) ~= (loss0(0) + (math.pow(initialWeightsWithIntercept(0), 2) +
...
class GradientDescentClusterSuite extends SparkFunSuite with LocalClusterSparkContext {
  ...
  test("task size should be small") {
    ...
    val (weights, loss) = GradientDescent.runMiniBatchSGD(points,
