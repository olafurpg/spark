...
class LBFGSSuite extends SparkFunSuite with MLlibTestSparkContext with Matchers {
  ...
  test("LBFGS loss should be decreasing and match the result of Gradient Descent.") {
    ...
    val (_, lossGD) = GradientDescent.runMiniBatchSGD(dataRDD,
                                                      gradient,
                                                      simpleUpdater,
                                                      stepSize,
                                                      numGDIterations,
  ...
  test("LBFGS and Gradient Descent with L2 regularization should get the same result.") {
    ...
    val numGDIterations = 50
    val stepSize = 1.0
    val (weightGD, lossGD) = GradientDescent.runMiniBatchSGD(dataRDD,
                                                             gradient,
                                                             squaredL2Updater,
                                                             stepSize,
                                                             numGDIterations,
  ...
  test("Optimize via class LBFGS.") {
    ...
    val numGDIterations = 50
    val stepSize = 1.0
    val (weightGD, _) = GradientDescent.runMiniBatchSGD(dataRDD,
                                                        gradient,
                                                        squaredL2Updater,
                                                        stepSize,
                                                        numGDIterations,
