...
class RidgeRegressionSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("ridge regression can help avoid overfitting") {
    ...
    val linearErr =
      predictionError(linearModel.predict(validationRDD.map(_.features)).collect(), validationData)
    ...
    val ridgeErr =
      predictionError(ridgeModel.predict(validationRDD.map(_.features)).collect(), validationData)
