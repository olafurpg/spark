...
class StreamingTestSuite extends SparkFunSuite with TestSuiteBase {
  ...
  test("accuracy for null hypothesis using welch t-test") {
    ...
    val model = new StreamingTest().setWindowSize(0).setPeacePeriod(0).setTestMethod(testMethod)
  ...
  test("null hypothesis when only data from one group is present") {
    ...
    val input = generateTestData(numBatches, pointsPerBatch, meanA, stdevA, meanB, stdevB, 42).map(
  ...
  private def generateTestData(numBatches: Int,
                               pointsPerBatch: Int,
                               meanA: Double,
                               stdevA: Double,
                               meanB: Double,
