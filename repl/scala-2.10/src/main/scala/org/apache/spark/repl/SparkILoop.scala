...
class SparkILoop(
  ...
  protected def createInterpreter() {
    ...
    val totalClassPath =
      addedJars.foldLeft(settings.classpath.value)((l, r) => ClassPath.join(l, r))
  ...
  private def ambiguousError(cmd: String): Result = {
    matchingCommands(cmd) match {
      ...
      case xs =>
        echo(cmd + " is ambiguous: did you mean " + xs.map(":" + _.name).mkString(" or ") + "?")
    }
    Result(true, None)
  }
  ...
  private lazy val historyCommand = new LoopCommand(
    ...
    def apply(line: String): Result = {
      ...
      for ((line, index) <- lines.zipWithIndex) echo("%3d  %s".format(index + offset, line))
    }
  }
  ...
  private def searchHistory(_cmdline: String) {
    ...
    for ((line, index) <- history.asStrings.zipWithIndex;
         if line.toLowerCase contains cmdline) echo("%d %s".format(index + offset, line))
  }
...
//    nullary("power", "enable power user mode", powerCmd),
  ...
  private def newJavap() =
    new JavapClass(addToolsJarToLoader(), new SparkIMain.ReplStrippingWriter(intp)) {
  ...
  private def crashRecovery(ex: Throwable): Boolean = {
    ...
    ex match {
      ...
      case _ =>
        def fn(): Boolean =
          try in.readYesOrNo(replayQuestionMessage, { echo("\nYou must enter y or n."); fn() }) catch {
...
//   }
  ...
  private def process(settings: Settings): Boolean = savingContextLoader {
    ...
    addThunk(intp.quietBind(
            NamedParam[SparkIMain]("$intp", intp)(tagOfSparkIMain, classTag[SparkIMain])))
    addThunk({
