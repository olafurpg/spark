...
class SparkIMain(initialSettings: Settings,
  ...
  private def updateCompilerClassPath(urls: URL*): Unit = {
    ...
    val fieldSetter =
      platform.getClass.getMethods.find(_.getName.endsWith("currentClassPath_$eq")).get
  ...
  private def mostRecentlyHandledTree: Option[Tree] = {
    prevRequests.reverse foreach { req =>
      req.handlers.reverse foreach {
        case x: MemberDefHandler if x.definesValue && !isInternalTermName(x.name) =>
  ...
  private def safePos(t: Tree, alt: Int): Int =
    try t.pos.startOrPoint catch { case _: UnsupportedOperationException => alt }
  ...
  def bind(name: String, boundType: String, value: Any, modifiers: List[String] = Nil): IR.Result = {
    ...
    bindRep.callEither("set", value) match {
      ...
      case Right(_) =>
        val line =
          "%sval %s = %s.value".format(modifiers map (_ + " ") mkString, name, bindRep.evalPath)
  ...
  private def directBind[T: ru.TypeTag: ClassTag](name: String, value: T): IR.Result =
  ...
  class ReadEvalPrint(val lineId: Int) {
    ...
    def callOpt(name: String, args: Any*): Option[AnyRef] =
      try Some(call(name, args: _*)) catch { case ex: Throwable => bindError(ex); None }
    ...
    private def evalMethod(name: String) = evalClass.getMethods filter (_.getName == name) match {
      ...
      case xs =>
        sys.error("Internal error: eval object " + evalClass + ", " + xs.mkString("\n", "\n", ""))
    }
  ...
  class Request(val line: String, val trees: List[Tree]) {
     ...
     */
    val SparkComputedImports(importsPreamble, importsTrailer, accessPath) = importsCode(
        referencedNames.toSet, definedClasses)
  ...
  def definedSymbolList =
    prevRequestList flatMap (_.definedSymbolList) filterNot (s => isInternalTermName(s.name))
