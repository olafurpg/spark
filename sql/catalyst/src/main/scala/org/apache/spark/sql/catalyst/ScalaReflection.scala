...
object ScalaReflection extends ScalaReflection {
  ...
  private def deserializerFor(
      tpe: `Type`, path: Option[Expression], walkedTypePath: Seq[String]): Expression =
    ScalaReflectionLock.synchronized {
      ...
      def addToPathOrdinal(
          ordinal: Int, dataType: DataType, walkedTypePath: Seq[String]): Expression = {
        val newPath = path
      ...
      def upCastToExpectedType(
          expr: Expression, expected: DataType, walkedTypePath: Seq[String]): Expression =
        expected match {
      ...
      tpe match {
        ...
        case t if t <:< localTypeOf[Array[_]] =>
          ...
          primitiveMethod.map { method =>
            ...
            Invoke(MapObjects(p => deserializerFor(elementType, Some(p), newTypePath),
                              getPath,
        ...
        case t if t <:< localTypeOf[Map[_, _]] =>
          ...
          val keyData =
            Invoke(MapObjects(p => deserializerFor(keyType, Some(p), walkedTypePath),
          ...
          val valueData = Invoke(
              MapObjects(p => deserializerFor(valueType, Some(p), walkedTypePath),
        ...
        case t if t.typeSymbol.annotations.exists(_.tpe =:= typeOf[SQLUserDefinedType]) =>
          ...
          val obj =
            NewInstance(udt.userClass.getAnnotation(classOf[SQLUserDefinedType]).udt(),
                        Nil,
                        dataType = ObjectType(
        ...
        case t if UDTRegistration.exists(getClassNameFromType(t)) =>
          ...
          val obj = NewInstance(udt.getClass, Nil, dataType = ObjectType(udt.getClass))
          Invoke(obj, "deserialize", ObjectType(udt.userClass), getPath :: Nil)
      }
    }
  ...
  private def serializerFor(
      inputObject: Expression, tpe: `Type`, walkedTypePath: Seq[String]): Expression =
    ScalaReflectionLock.synchronized {
      ...
      if (!inputObject.dataType.isInstanceOf[ObjectType]) {
        ...
        tpe match {
          case t if t <:< localTypeOf[Option[_]] =>
            ...
            optType match {
              ...
              case t if t <:< definitions.IntTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Integer]), inputObject),
                       "intValue",
                       IntegerType)
              case t if t <:< definitions.LongTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Long]), inputObject),
                       "longValue",
                       LongType)
              case t if t <:< definitions.DoubleTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Double]), inputObject),
                       "doubleValue",
                       DoubleType)
              case t if t <:< definitions.FloatTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Float]), inputObject),
                       "floatValue",
                       FloatType)
              case t if t <:< definitions.ShortTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Short]), inputObject),
                       "shortValue",
                       ShortType)
              case t if t <:< definitions.ByteTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Byte]), inputObject),
                       "byteValue",
                       ByteType)
              case t if t <:< definitions.BooleanTpe =>
                Invoke(UnwrapOption(ObjectType(classOf[java.lang.Boolean]), inputObject),
                       "booleanValue",
                       BooleanType)
          ...
          case t if t <:< localTypeOf[Map[_, _]] =>
            ...
            val keys = Invoke(Invoke(inputObject,
                                     "keysIterator",
            ...
            val values = Invoke(Invoke(inputObject,
                                       "valuesIterator",
                                       ObjectType(classOf[scala.collection.Iterator[_]])),
                                "toSeq",
                                ObjectType(classOf[scala.collection.Seq[_]]))
  ...
  def schemaFor(tpe: `Type`): Schema = ScalaReflectionLock.synchronized {
    tpe match {
      ...
      case t if t <:< localTypeOf[Map[_, _]] =>
        ...
        val Schema(valueDataType, valueNullable) = schemaFor(valueType)
        Schema(
            MapType(schemaFor(keyType).dataType, valueDataType, valueContainsNull = valueNullable),
            nullable = true)
      case t if definedByConstructorParams(t) =>
        val params = getConstructorParameters(t)
  ...
  protected def constructParams(tpe: Type): Seq[Symbol] = {
    ...
    val params =
      if (constructorSymbol.isMethod) {
        ...
        val primaryConstructorSymbol: Option[Symbol] = constructorSymbol.asTerm.alternatives
