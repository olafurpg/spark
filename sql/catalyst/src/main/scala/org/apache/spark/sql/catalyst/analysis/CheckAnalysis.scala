      ...
      case operator: LogicalPlan =>
          ...
          case j @ Join(_, _, _, Some(condition)) =>
            def checkValidJoinConditionExprs(expr: Expression): Unit = expr match {
              ...
              case e if e.dataType.isInstanceOf[BinaryType] =>
                failAnalysis(
                    s"binary type expression ${e.sql} cannot be used " + "in join conditions")
              case e if e.dataType.isInstanceOf[MapType] =>
                failAnalysis(
                    s"map type expression ${e.sql} cannot be used " + "in join conditions")
              case _ => // OK
            }
        ...
        operator match {
          case o if o.children.nonEmpty && o.missingInput.nonEmpty =>
            ...
            failAnalysis(s"resolved attribute(s) $missingAttributes missing from $input " +
          ...
          case o
              if o.expressions.exists(!_.deterministic) && !o.isInstanceOf[Project] &&
              !o.isInstanceOf[Filter] && !o.isInstanceOf[Aggregate] && !o.isInstanceOf[Window] =>
