...
trait String2StringExpression extends ImplicitCastInputTypes { self: UnaryExpression =>
...
trait StringPredicate extends Predicate with ImplicitCastInputTypes { self: BinaryExpression =>
...
object StringTranslate {
  ...
  def buildDict(
      matchingString: UTF8String, replaceString: UTF8String): JMap[Character, Character] = {
...
@ExpressionDescription(
    ...
    extended = "> SELECT _FUNC_('    SparkSQL   ');\n 'SparkSQL'")
case class StringTrim(child: Expression) extends UnaryExpression with String2StringExpression {
 ...
 */
@ExpressionDescription(usage = "_FUNC_(str) - Removes the leading space characters from str.",
                       extended = "> SELECT _FUNC_('    SparkSQL   ');\n 'SparkSQL   '")
case class StringTrimLeft(child: Expression) extends UnaryExpression with String2StringExpression {
 ...
 */
@ExpressionDescription(usage = "_FUNC_(str) - Removes the trailing space characters from str.",
                       extended = "> SELECT _FUNC_('    SparkSQL   ');\n '    SparkSQL'")
    ...
    if (s == null) {
      ...
      if (r == null) {
        ...
        if (l == null) {
          ...
          l.asInstanceOf[UTF8String].indexOf(r.asInstanceOf[UTF8String], s.asInstanceOf[Int]) + 1
        }
      }
    }
  }
 ...
 */
@ExpressionDescription(
    usage = """_FUNC_(str) - Returns str with the first letter of each word in uppercase.
 ...
 */
@ExpressionDescription(usage = "_FUNC_(str) - Returns the reversed given string.",
                       extended = "> SELECT _FUNC_('Spark SQL');\n 'LQS krapS'")
 ...
 */
@ExpressionDescription(
    usage = "_FUNC_(n) - Returns a n spaces string.", extended = "> SELECT _FUNC_(2);\n '  '")
    ...
    str.dataType match {
      ...
      case BinaryType =>
        ByteArray.subStringSQL(
            string.asInstanceOf[Array[Byte]], pos.asInstanceOf[Int], len.asInstanceOf[Int])
    }
  }
...
case class Levenshtein(left: Expression, right: Expression)
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    nullSafeCodeGen(ctx, ev, (left, right) => s"${ev.value} = $left.levenshteinDistance($right);")
  }
}
 ...
 */
@ExpressionDescription(usage = "_FUNC_(str) - Returns soundex code of the string.",
...
case class Base64(child: Expression) extends UnaryExpression with ImplicitCastInputTypes {
  ...
  protected override def nullSafeEval(bytes: Any): Any = {
    UTF8String.fromBytes(
        org.apache.commons.codec.binary.Base64.encodeBase64(bytes.asInstanceOf[Array[Byte]]))
  }
...
case class FormatNumber(x: Expression, d: Expression)
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    nullSafeCodeGen(ctx, ev, (num, d) => {
      ...
      ctx.addMutableState(
          df, numberFormat, s"""$numberFormat = new $df("", new $dfs($l.$usLocale));""")
