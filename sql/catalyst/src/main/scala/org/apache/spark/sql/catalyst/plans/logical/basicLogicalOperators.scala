...
case class Generate(generator: Generator,
  ...
  override lazy val resolved: Boolean = {
    ...
    generator.elementSchema.length == generatorOutput.length && generatorOutput.forall(_.resolved)
  }
...
case class Filter(condition: Expression, child: LogicalPlan)
  ...
  override protected def validConstraints: Set[Expression] = {
    val predicates =
      splitConjunctivePredicates(condition).filterNot(SubqueryExpression.hasCorrelatedSubquery)
...
case class InsertIntoTable(table: LogicalPlan,
  ...
  private[spark] lazy val expectedColumns = {
    if (table.output.isEmpty) {
      ...
      val (partitionColumns, dataColumns) =
        table.output.partition(a => partition.keySet.contains(a.name))
  ...
  override lazy val resolved: Boolean =
    childrenResolved && expectedColumns.forall { expected =>
...
case class Range(start: Long, end: Long, step: Long, numSlices: Int, output: Seq[Attribute])
    extends LeafNode
...
private[sql] object Expand {
   ...
   */
  private def buildNonSelectAttrSet(bitmask: Int, attrs: Seq[Attribute]): AttributeSet = {
