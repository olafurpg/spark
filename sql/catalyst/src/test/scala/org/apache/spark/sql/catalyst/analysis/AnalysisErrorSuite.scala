...
class AnalysisErrorSuite extends AnalysisTest {
  ...
  errorTest("single invalid type, second arg",
            testRelation.select(
  ...
  errorTest("multiple invalid type",
            testRelation.select(
  ...
  errorTest("unresolved attributes", testRelation.select('abcd), "cannot resolve" :: "abcd" :: Nil)

  errorTest("unresolved attributes with a generated name",
            testRelation2.groupBy('a)(max('b)).where(sum('b) > 0).orderBy('havingCondition.asc),
            "cannot resolve" :: "havingCondition" :: Nil)

  errorTest("unresolved star expansion in max",
  ...
  errorTest("sorting by attributes are not from grouping expressions",
            testRelation2.groupBy('a, 'c)('a, 'c, count('a).as("a3")).orderBy('b.asc),
            "cannot resolve" :: "'`b`'" :: "given input columns" :: "[a, c, a3]" :: Nil)

  errorTest("non-boolean filters",
            testRelation.where(Literal(1)),
            "filter" :: "'1'" :: "not a boolean" :: Literal(1).dataType.simpleString :: Nil)

  errorTest("non-boolean join conditions",
  ...
  errorTest("ambiguous field due to case insensitivity",
  ...
  errorTest("catch all unresolved plan", UnresolvedTestPlan(), "unresolved" :: Nil)

  errorTest("union with unequal number of columns",
  ...
  errorTest(
      ...
      testRelation2.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "1 second", "2 second", "0 second").as("window")),
  ...
  errorTest(
      ...
      testRelation.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "1 second", "1 second", "1 minute").as("window")),
  ...
  errorTest(
      ...
      testRelation.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "1 second", "1 second", "1 second").as("window")),
      "The start time " :: " must be less than the slideDuration " :: Nil
  )
  ...
  errorTest(
      ...
      testRelation.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "-1 second", "1 second", "0 second").as("window")),
  ...
  errorTest(
      ...
      testRelation.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "0 second", "1 second", "0 second").as("window")),
      "The window duration " :: " must be greater than 0." :: Nil
  )
  ...
  errorTest(
      ...
      testRelation.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "1 second", "0 second", "0 second").as("window")),
      "The slide duration" :: " must be greater than 0." :: Nil
  )
  ...
  errorTest(
      ...
      testRelation.select(TimeWindow(
              Literal("2016-01-01 01:01:01"), "1 second", "1 second", "-5 second").as("window")),
  ...
  errorTest(
      ...
      "Generators are not supported when it's nested in expressions, but got: (explode(list) + 1)" :: Nil
  )
  ...
  test("check grouping expression data types") {
    ...
    val unsupportedDataTypes = Seq(MapType(StringType, LongType),
                                   new StructType()
                                     .add("f1", FloatType, nullable = true)
  ...
  test("Join can't work on binary and map types") {
    val plan = Join(LocalRelation(AttributeReference("a", BinaryType)(exprId = ExprId(2)),
                                  AttributeReference("b", IntegerType)(exprId = ExprId(1))),
    ...
    assertAnalysisError(
        plan, "binary type expression `a` cannot be used in join conditions" :: Nil)
  ...
  test("PredicateSubQuery is used outside of a filter") {
    ...
    val plan =
      Project(Seq(a, Alias(In(a, Seq(ListQuery(LocalRelation(b)))), "c")()), LocalRelation(a))
  ...
  test("PredicateSubQuery is used is a nested condition") {
    ...
    val plan1 =
      Filter(Cast(Not(In(a, Seq(ListQuery(LocalRelation(b))))), BooleanType), LocalRelation(a))
    assertAnalysisError(
        plan1, "Null-aware predicate sub-queries cannot be used in nested conditions" :: Nil)

    val plan2 = Filter(Or(Not(In(a, Seq(ListQuery(LocalRelation(b))))), c), LocalRelation(a, c))
    assertAnalysisError(
        plan2, "Null-aware predicate sub-queries cannot be used in nested conditions" :: Nil)
  }
  ...
  test("PredicateSubQuery correlated predicate is nested in an illegal plan") {
    ...
    val plan1 = Filter(Exists(
                           Join(LocalRelation(b),
    ...
    val plan2 = Filter(Exists(
                           Join(Filter(EqualTo(OuterReference(a), c), LocalRelation(c)),
                                LocalRelation(b),
                                RightOuter,
                                Option(EqualTo(b, c)))),
                       LocalRelation(a))
