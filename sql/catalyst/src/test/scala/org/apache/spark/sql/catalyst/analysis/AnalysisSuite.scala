...
class AnalysisSuite extends AnalysisTest {
  ...
  test("union project *") {
    val plan = (1 to 120).map(_ => testRelation).fold[LogicalPlan](testRelation) { (a, b) =>
  ...
  test("analyze project") {
    ...
    checkAnalysis(Project(Seq(UnresolvedAttribute("TbL.a")),
                          UnresolvedRelation(TableIdentifier("TaBlE"), Some("TbL"))),
                  Project(testRelation.output, testRelation))

    assertAnalysisError(Project(Seq(UnresolvedAttribute("tBl.a")),
                                UnresolvedRelation(TableIdentifier("TaBlE"), Some("TbL"))),
                        Seq("cannot resolve"))
  ...
  test("resolve sort references - join") {
    ...
    val plan =
      testRelation2.join(testRelation3).where('a > "str").select('a, 'b).sortBy('c.desc, 'h.asc)
  ...
  test("resolve sort references - aggregate") {
    ...
    val plan2 = testRelation2.groupBy('a, 'c, 'b)('a, 'c, count('a).as("a3")).orderBy('b.asc)
  ...
  test("pull out nondeterministic expressions from Sort") {
    ...
    val projected = Alias(Rand(33), "_nondeterministic")()
    val expected = Project(testRelation.output,
  ...
  test("SPARK-8654: invalid CAST in NULL IN(...) expression") {
    val plan =
      Project(Alias(In(Literal(null), Seq(Literal(1), Literal(2))), "a")() :: Nil, LocalRelation())
    assertAnalysisSuccess(plan)
  }

  test("SPARK-8654: different types in inlist but can be converted to a common type") {
    val plan = Project(
        Alias(In(Literal(null), Seq(Literal(1), Literal(1.2345))), "a")() :: Nil, LocalRelation())
    assertAnalysisSuccess(plan)
  }

  test("SPARK-8654: check type compatibility error") {
    val plan = Project(
        Alias(In(Literal(null), Seq(Literal(true), Literal(1))), "a")() :: Nil, LocalRelation())
