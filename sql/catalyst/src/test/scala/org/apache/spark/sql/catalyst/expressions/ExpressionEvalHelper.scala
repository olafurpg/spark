 ...
 */
trait ExpressionEvalHelper extends GeneratorDrivenPropertyChecks { self: SparkFunSuite =>
  ...
  protected def generateProject(generator: => Projection, expression: Expression): Projection = {
    try {
      generator
    } catch {
      case e: Throwable =>
  ...
  protected def checkDoubleEvaluation(expression: => Expression,
    ...
    plan = generateProject(
        ...
        expression)
    actual =
      FromUnsafeProjection(expression.dataType :: Nil)(plan(inputRow)).get(0, expression.dataType)
    assert(checkResult(actual, expected))
  }
   ...
   * and `dataType2`.
   */
  def checkConsistencyBetweenInterpretedAndCodegen(c: (Expression, Expression) => Expression,
                                                   dataType1: DataType,
   ...
   */
  def checkConsistencyBetweenInterpretedAndCodegen(
      c: Seq[Expression] => Expression, dataType: DataType, minNumElements: Int = 0): Unit = {
  ...
  private def cmpInterpretWithCodegen(inputRow: InternalRow, expr: Expression): Unit = {
    ...
    val plan = generateProject(
        GenerateMutableProjection.generate(Alias(expr, s"Optimized($expr)")() :: Nil), expr)
