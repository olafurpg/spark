...
class MiscFunctionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("md5") {
    checkEvaluation(
        Md5(Literal("ABC".getBytes(StandardCharsets.UTF_8))), "902fbdd2b1df0c4f70b4a5d23525e932")
  ...
  test("sha2") {
    ...
    checkEvaluation(
        ...
        null)
    checkEvaluation(
        Sha2(Literal.create(null, BinaryType), Literal.create(null, IntegerType)), null)
  }
