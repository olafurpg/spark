...
class GeneratedProjectionSuite extends SparkFunSuite {
  ...
  test("generated projections on wider table") {
    ...
    val wideRow2 =
      new GenericInternalRow((1 to N).map(i => UTF8String.fromString(i.toString)).toArray[Any])
    ...
    val nestedSchema =
      StructType(Seq(StructField("", joinedSchema), StructField("", joinedSchema)) ++ joinedSchema)
  ...
  test("generated unsafe projection with array of binary") {
    val row = InternalRow(Array[Byte](1, 2),
