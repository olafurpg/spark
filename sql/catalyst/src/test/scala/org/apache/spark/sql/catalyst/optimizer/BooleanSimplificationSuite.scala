...
class BooleanSimplificationSuite extends PlanTest with PredicateHelper {
  ...
  test("(a || b || c || ...) && (a || b || d || ...) && (a || b || e || ...) ...") {
    ...
    checkCondition(('a === 'b || 'b > 3) && ('a === 'b || 'a > 3) && ('a === 'b || 'a < 5),
                   'a === 'b || 'b > 3 && 'a > 3 && 'a < 5)
  }
  ...
  test("(a && b) || (a && c) => a && (b || c) when case insensitive") {
    val plan =
      caseInsensitiveAnalyzer.execute(testRelation.where(('a > 2 && 'b > 3) || ('A > 2 && 'b < 5)))
    val actual = Optimize.execute(plan)
    val expected =
      caseInsensitiveAnalyzer.execute(testRelation.where('a > 2 && ('b > 3 || 'b < 5)))
