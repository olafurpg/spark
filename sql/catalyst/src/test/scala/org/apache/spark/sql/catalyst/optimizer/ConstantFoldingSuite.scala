...
class ConstantFoldingSuite extends PlanTest {
  ...
  test("Constant folding test: expressions only have literals") {
    val originalQuery = testRelation
      .select(Literal(2) + Literal(3) + Literal(4) as Symbol("2+3+4"),
              ...
              Literal(2) * (Literal(3) + Literal(4)) as Symbol("2*(3+4)"))
      .where(Literal(1) === Literal(1) && Literal(2) > Literal(3) || Literal(3) > Literal(2))
      .groupBy(
  ...
  test("Constant folding test: expressions have attribute references and literals in " +
    ...
    val originalQuery = testRelation.select(Literal(2) + Literal(3) + 'a as Symbol("c1"),
  ...
  test("Constant folding test: expressions have attribute references and literals in " +
    ...
    val originalQuery = testRelation.where(
        (('a > 1 && Literal(1) === Literal(1)) || ('a < 10 && Literal(1) === Literal(2)) ||
            (Literal(1) === Literal(1) && 'b > 1) || (Literal(1) === Literal(2) && 'b < 10)) &&
        (('a > 1 || Literal(1) === Literal(1)) && ('a < 10 || Literal(1) === Literal(2)) &&
            (Literal(1) === Literal(1) || 'b > 1) && (Literal(1) === Literal(2) || 'b < 10)))

    val optimized = Optimize.execute(originalQuery.analyze)

    val correctAnswer = testRelation.where(('a > 1 || 'b > 1) && ('a < 10 && 'b < 10)).analyze
  ...
  test("Constant folding test: expressions have foldable functions") {
    ...
    val correctAnswer =
      testRelation.select(Literal(5) + 'a as Symbol("c1"), Literal(3) as Symbol("c2")).analyze
  ...
  test("Constant folding test: expressions have nonfoldable functions") {
    val originalQuery =
      testRelation.select(Rand(5L) + Literal(1) as Symbol("c1"), sum('a) as Symbol("c2"))

    val optimized = Optimize.execute(originalQuery.analyze)

    val correctAnswer =
      testRelation.select(Rand(5L) + Literal(1.0) as Symbol("c1"), sum('a) as Symbol("c2")).analyze
  ...
  test("Constant folding test: Fold In(v, list) into true or false") {
    val originalQuery = testRelation.select('a).where(In(Literal(1), Seq(Literal(1), Literal(2))))
