...
class FilterPushdownSuite extends PlanTest {
  ...
  test("combine redundant filters") {
    val originalQuery = testRelation.where('a === 1 && 'b === 1).where('a === 1 && 'c === 1)

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a === 1 && 'b === 1 && 'c === 1).analyze

    comparePlans(optimized, correctAnswer)
  }

  test("can't push without rewrite") {
    val originalQuery = testRelation.select('a + 'b as 'e).where('e === 1).analyze

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a + 'b === 1).select('a + 'b as 'e).analyze
  ...
  test("nondeterministic: can't push down filter with nondeterministic condition through project") {
    val originalQuery =
      testRelation.select(Rand(10).as('rand), 'a).where('rand > 5 || 'a > 5).analyze
  ...
  test("nondeterministic: can't push down filter through project with nondeterministic field") {
    val originalQuery = testRelation.select(Rand(10).as('rand), 'a).where('a > 5).analyze
  ...
  test("filters: combines filters") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a === 1 && 'a === 2).select('a).analyze
  ...
  test("joins: push down left semi join") {
    ...
    val correctAnswer = left.join(right, LeftSemi, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: push down right outer join #1") {
    ...
    val correctAnswer = x.join(right, RightOuter).where("x.b".attr === 1).analyze
  ...
  test("joins: push down left outer join #2") {
    ...
    val originalQuery = {
      x.join(y, LeftOuter, Some("x.b".attr === 1)).where("x.b".attr === 2 && "y.b".attr === 2)
    }
  ...
  test("joins: push down right outer join #2") {
    ...
    val originalQuery = {
      x.join(y, RightOuter, Some("y.b".attr === 1)).where("x.b".attr === 2 && "y.b".attr === 2)
    }
  ...
  test("joins: push down left outer join #3") {
    ...
    val originalQuery = {
      x.join(y, LeftOuter, Some("y.b".attr === 1)).where("x.b".attr === 2 && "y.b".attr === 2)
    }

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where('b === 2).subquery('l)
    val right = testRelation.where('b === 1).subquery('r)
    val correctAnswer = left.join(right, LeftOuter).where("r.b".attr === 2).analyze
  ...
  test("joins: conjunctive predicates") {
    ...
    val originalQuery = {
      x.join(y).where(("x.b".attr === "y.b".attr) && ("x.a".attr === 1) && ("y.a".attr === 1))
    }

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where('a === 1).subquery('x)
    val right = testRelation.where('a === 1).subquery('y)
    val correctAnswer = left.join(right, condition = Some("x.b".attr === "y.b".attr)).analyze
  ...
  test("joins: conjunctive predicates #3") {
    ...
    val originalQuery = {
      z.join(x.join(y))
        .where(("x.b".attr === "y.b".attr) && ("x.a".attr === 1) && ("z.a".attr >= 3) &&
            ("z.a".attr === "x.b".attr))
    }
  ...
  test("aggregate: push down filter when filter on group by expression") {
    val originalQuery =
      testRelation.groupBy('a)('a, count('b) as 'c).select('a, 'c).where('a === 2)

    val optimized = Optimize.execute(originalQuery.analyze)

    val correctAnswer = testRelation.where('a === 2).groupBy('a)('a, count('b) as 'c).analyze
    comparePlans(optimized, correctAnswer)
  }

  test("aggregate: don't push down filter when filter not on group by expression") {
    val originalQuery =
      testRelation.select('a, 'b).groupBy('a)('a, count('b) as 'c).where('c === 2L)
  ...
  test("aggregate: push down filters partially which are subset of group by expressions") {
    val originalQuery =
      testRelation.select('a, 'b).groupBy('a)('a, count('b) as 'c).where('c === 2L && 'a === 3)
  ...
  test("broadcast hint") {
    val originalQuery =
      BroadcastHint(testRelation).where('a === 2L && 'b + Rand(10).as("rnd") === 3)
  ...
  test("union") {
    ...
    val originalQuery =
      Union(Seq(testRelation, testRelation2)).where('a === 2L && 'b + Rand(10).as("rnd") === 3)

    val optimized = Optimize.execute(originalQuery.analyze)

    val correctAnswer = Union(Seq(testRelation.where('a === 2L), testRelation2.where('d === 2L)))
  ...
  test("expand") {
    val agg =
      testRelation.groupBy(Cube(Seq('a, 'b)))('a, 'b, sum('c)).analyze.asInstanceOf[Aggregate]
  ...
  test("Window: predicate push down -- multi window expressions with the same window spec") {
    ...
    val originalQuery =
      testRelation.select('a, 'b, 'c, winExpr1.as('window1), winExpr2.as('window2)).where('a > 1)
  ...
  test("Window: predicate push down -- multi window specification - 2") {
    ...
    val originalQuery =
      testRelation.select('a, winExpr1.as('window1), 'b, 'c, winExpr2.as('window2)).where('b > 1)
  ...
  ignore("Window: predicate push down -- complex predicate with the same expressions") {
    val winSpec = windowSpec(partitionSpec = 'a.attr + 'b.attr :: Nil,
                             orderSpec = 'b.asc :: Nil,
                             UnspecifiedFrame)
    val winExpr = windowExpr(count('b), winSpec)

    val winSpecAnalyzed =
      windowSpec(partitionSpec = '_w0.attr :: Nil, orderSpec = 'b.asc :: Nil, UnspecifiedFrame)
  ...
  test("Window: no predicate push down -- partial compound partition key") {
    val winSpec = windowSpec(partitionSpec = 'a.attr + 'b.attr :: 'b.attr :: Nil,
                             orderSpec = 'b.asc :: Nil,
                             UnspecifiedFrame)
    val winExpr = windowExpr(count('b), winSpec)
    ...
    val winSpecAnalyzed = windowSpec(partitionSpec = '_w0.attr :: 'b.attr :: Nil,
                                     orderSpec = 'b.asc :: Nil,
                                     UnspecifiedFrame)
    val winExprAnalyzed = windowExpr(count('b), winSpecAnalyzed)
    val correctAnswer = testRelation
