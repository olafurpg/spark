...
class LikeSimplificationSuite extends PlanTest {
  ...
  test("simplify Like into StartsWith") {
    ...
    val correctAnswer = testRelation.where(StartsWith('a, "abc") || ('a like "abc\\%")).analyze
  ...
  test("simplify Like into startsWith and EndsWith") {
    val originalQuery = testRelation.where(('a like "abc\\%def") || ('a like "abc%def"))
  ...
  test("simplify Like into Contains") {
    val originalQuery = testRelation.where(('a like "%mn%") || ('a like "%mn\\%"))

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where(Contains('a, "mn") || ('a like "%mn\\%")).analyze
  ...
  test("simplify Like into EqualTo") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where(('a === "") || ('a === "abc")).analyze
