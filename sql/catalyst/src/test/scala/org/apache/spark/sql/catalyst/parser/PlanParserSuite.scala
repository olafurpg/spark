...
class PlanParserSuite extends PlanTest {
  ...
  test("common table expressions") {
    ...
    assertEqual("with cte1 as (select * from a) select * from cte1",
                cte(table("cte1").select(star()), "cte1" -> table("a").select(star())))
    assertEqual("with cte1 (select 1) select * from cte1",
                cte(table("cte1").select(star()), "cte1" -> OneRowRelation.select(1)))
    assertEqual("with cte1 (select 1), cte2 as (select * from cte1) select * from cte2",
                cte(table("cte2").select(star()),
                    "cte1" -> OneRowRelation.select(1),
                    "cte2" -> table("cte1").select(star())))
    intercept("with cte1 (select 1), cte1 as (select 1 from cte1) select * from cte1",
  ...
  test("simple select query") {
    ...
    assertEqual(
        "select a, b from db.c having x < 1", table("db", "c").select('a, 'b).where('x < 1))
  ...
  test("reverse select query") {
    ...
    assertEqual("from db.a select b, c where d < 1", table("db", "a").where('d < 1).select('b, 'c))
  ...
  test("multi select query") {
    assertEqual("from a select * select * where s < 10",
                table("a").select(star()).union(table("a").where('s < 10).select(star())))
    intercept(
        "from a select * select * from x where a.s < 10",
        "Multi-Insert queries cannot have a FROM clause in their individual SELECT statements")
    assertEqual("from a insert into tbl1 select * insert into tbl2 select * where s < 10",
                table("a")
                  .select(star())
                  .insertInto("tbl1")
  ...
  test("insert into") {
    ...
    def insert(partition: Map[String, Option[String]],
    ...
    assertEqual(
        ...
        InsertIntoTable(
            ...
            InsertIntoTable(table("u"), Map.empty, plan2, overwrite = false, ifNotExists = false)))
  }
  ...
  test("aggregation") {
    ...
    assertEqual(s"$sql grouping sets((a, b), (a), ())",
                GroupingSets(
                    Seq(0, 1, 3), Seq('a, 'b), table("d"), Seq('a, 'b, 'sum.function('c).as("c"))))
    intercept(s"$sql grouping sets((a, b), (c), ())", "c doesn't show up in the GROUP BY list")
  }
  ...
  test("lateral view") {
    ...
    // Single lateral view
    assertEqual("select * from t lateral view explode(x) expl as x",
                table("t")
    ...
    val expected = table("t")
      .generate(UnresolvedGenerator(FunctionIdentifier("posexplode"), Seq('x)),
                join = true,
                outer = false,
                Some("posexpl"),
                Seq("x", "y"))
  ...
  test("sub-query") {
    ...
    assertEqual("select id from ((((((t0))))))", plan)
    assertEqual("(select * from t1) union distinct (select * from t2)",
                Distinct(table("t1").select(star()).union(table("t2").select(star()))))
    assertEqual("select * from ((select * from t1) union (select * from t2)) t",
                Distinct(table("t1").select(star()).union(table("t2").select(star())))
                  .as("t")
                  .select(star()))
    assertEqual("""select  id
  ...
  test("scalar sub-query") {
    assertEqual("select (select max(b) from s) ss from t",
                table("t").select(ScalarSubquery(table("s").select('max.function('b))).as("ss")))
    assertEqual("select * from t where a = (select b from s)",
                table("t").where('a === ScalarSubquery(table("s").select('b))).select(star()))
    assertEqual("select g from t group by g having a > (select b from s)",
  ...
  test("inline table") {
    assertEqual("values 1, 2, 3, 4",
                LocalRelation.fromExternalRows(Seq('col1.int), Seq(1, 2, 3, 4).map(x => Row(x))))
    assertEqual("values (1, 'a'), (2, 'b'), (3, 'c') as tbl(a, b)",
                LocalRelation
                  .fromExternalRows(Seq('a.int, 'b.string),
                                    Seq((1, "a"), (2, "b"), (3, "c")).map(x => Row(x._1, x._2)))
                  .as("tbl"))
    intercept("values (a, 'a'), (b, 'b')", "All expressions in an inline table must be constants.")
