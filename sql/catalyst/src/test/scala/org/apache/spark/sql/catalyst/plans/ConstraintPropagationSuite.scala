...
class ConstraintPropagationSuite extends SparkFunSuite {
  ...
  test("propagating constraints in filters") {
    ...
    verifyConstraints(
        tr.where('a.attr > 10).select('c.attr, 'a.attr).where('c.attr =!= 100).analyze.constraints,
        ExpressionSet(
  ...
  test("propagating constraints in expand") {
    ...
    verifyConstraints(notNullRelation.analyze.constraints,
                      ExpressionSet(
                          Seq(resolveColumn(notNullRelation.analyze, "c") > 10,
    ...
    val expand = Expand(Seq(Seq('c, Literal.create(null, StringType), 1), Seq('c, 'a, 2)),
                        Seq('c, 'a, 'gid.int),
  ...
  test("propagating constraints in union") {
    ...
    assert(
        tr1
          .where('a.attr > 10)
          .union(tr2.where('e.attr > 10).union(tr3.where('i.attr > 10)))
          .analyze
          .constraints
          .isEmpty)
    ...
    verifyConstraints(
        tr1
          .where('a.attr > 10)
          .union(tr2.where('d.attr > 10).union(tr3.where('g.attr > 10)))
          .analyze
          .constraints,
        ExpressionSet(Seq(resolveColumn(tr1, "a") > 10, IsNotNull(resolveColumn(tr1, "a")))))

    val a = resolveColumn(tr1, "a")
    verifyConstraints(tr1.where('a.attr > 10).union(tr2.where('d.attr > 11)).analyze.constraints,
  ...
  test("propagating constraints in intersect") {
    ...
    verifyConstraints(
        tr1.where('a.attr > 10).intersect(tr2.where('b.attr < 100)).analyze.constraints,
        ExpressionSet(
  ...
  test("propagating constraints in except") {
    ...
    val tr2 = LocalRelation('a.int, 'b.int, 'c.int)
    verifyConstraints(
        tr1.where('a.attr > 10).except(tr2.where('b.attr < 100)).analyze.constraints,
        ExpressionSet(Seq(resolveColumn(tr1, "a") > 10, IsNotNull(resolveColumn(tr1, "a")))))
  }
  ...
  test("infer additional constraints in filters") {
    ...
    verifyConstraints(tr.where('a.attr > 10 && 'a.attr === 'b.attr).analyze.constraints,
                      ExpressionSet(
                          Seq(resolveColumn(tr, "a") > 10,
  ...
  test("infer constraints on cast") {
    ...
    verifyConstraints(
        tr.where('a.attr === 'b.attr && 'c.attr + 100 > 'd.attr &&
              IsNotNull(Cast(Cast(resolveColumn(tr, "e"), LongType), LongType)))
          .analyze
          .constraints,
        ExpressionSet(
            Seq(Cast(resolveColumn(tr, "a"), LongType) === resolveColumn(tr, "b"),
  ...
  test("infer isnotnull constraints from compound expressions") {
    ...
    verifyConstraints(
        ...
        ExpressionSet(
            Seq(Cast(resolveColumn(tr, "a"), LongType) + resolveColumn(tr, "b") ===
                ...
                IsNotNull(resolveColumn(tr, "e")),
                IsNotNull(
                    Cast(Cast(Cast(resolveColumn(tr, "e"), LongType), LongType), LongType)))))
    ...
    verifyConstraints(tr.where('a.attr === 'c.attr && IsNotNull(IsNotNull(resolveColumn(tr, "b"))))
                        .analyze
                        .constraints,
                      ExpressionSet(
                          Seq(resolveColumn(tr, "a") === resolveColumn(tr, "c"),
