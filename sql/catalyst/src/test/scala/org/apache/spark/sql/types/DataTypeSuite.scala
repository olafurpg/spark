...
class DataTypeSuite extends SparkFunSuite {
  ...
  test("construct with String DataType") {
    ...
    val struct =
      (new StructType).add("a", "int", true).add("b", "long", false).add("c", "string", true)
  ...
  test("extract fields from a StructType") {
    ...
    val expectedStruct = StructType(StructField("b", LongType, false) ::
        StructField("d", FloatType, true) :: Nil)
  ...
  test("extract field index from a StructType") {
    val struct = StructType(StructField("a", LongType) ::
  ...
  test("merge where right is empty") {
    val left = StructType(StructField("a", LongType) ::
        StructField("b", FloatType) :: Nil)
  ...
  test("merge where left is empty") {
    ...
    val right = StructType(StructField("a", LongType) ::
        StructField("b", FloatType) :: Nil)
  ...
  def checkEqualsIgnoreCompatibleNullability(
    ...
    val testName = s"equalsIgnoreCompatibleNullability: (from: ${from}, to: ${to})"
    test(testName) {
      assert(DataType.equalsIgnoreCompatibleNullability(from, to) === expected)
    }
  }

  checkEqualsIgnoreCompatibleNullability(from = ArrayType(DoubleType, containsNull = true),
                                         to = ArrayType(DoubleType, containsNull = true),
                                         expected = true)
  checkEqualsIgnoreCompatibleNullability(from = ArrayType(DoubleType, containsNull = false),
  ...
  checkEqualsIgnoreCompatibleNullability(
      ...
      expected = false)
  checkEqualsIgnoreCompatibleNullability(
      from = StructType(StructField("a", StringType, nullable = false) ::
            StructField("b", StringType, nullable = true) :: Nil),
      to = StructType(StructField("a", StringType, nullable = false) ::
