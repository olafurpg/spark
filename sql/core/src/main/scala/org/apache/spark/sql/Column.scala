 ...
 * @since 1.6.0
 */
class TypedColumn[-T, U](expr: Expression, private[sql] val encoder: ExpressionEncoder[U])
    extends Column(expr) {
  ...
  private[sql] def withInputType(
      inputDeserializer: Expression, inputAttributes: Seq[Attribute]): TypedColumn[T, U] = {
