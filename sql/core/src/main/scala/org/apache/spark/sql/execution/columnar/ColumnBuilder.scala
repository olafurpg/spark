...
private[columnar] class BasicColumnBuilder[JvmType](
  ...
  override def initialize(
      initialSize: Int, columnName: String = "", useCompression: Boolean = false): Unit = {
...
private[columnar] abstract class NativeColumnBuilder[T <: AtomicType](
    override val columnStats: ColumnStats, override val columnType: NativeColumnType[T])
...
private[columnar] class ByteColumnBuilder extends NativeColumnBuilder(new ByteColumnStats, BYTE)
...
private[columnar] object ColumnBuilder {
  ...
  private[columnar] def ensureFreeSpace(orig: ByteBuffer, size: Int) = {
    if (orig.remaining >= size) {
      ...
      ByteBuffer.allocate(newSize).order(ByteOrder.nativeOrder()).put(orig.array(), 0, pos)
    }
  }
