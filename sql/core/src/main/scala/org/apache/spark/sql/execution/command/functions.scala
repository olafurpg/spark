 ...
 * }}}
 */
case class CreateFunction(databaseName: Option[String],
                          functionName: String,
                          className: String,
 ...
 * }}}
 */
case class DescribeFunction(functionName: FunctionIdentifier, isExtended: Boolean)
    extends RunnableCommand {
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    functionName.funcName.toLowerCase match {
      ...
      case "between" =>
        ...
        Row(s"Usage: a [NOT] BETWEEN b AND c - " + s"evaluate if a is [not] in between b and c") :: Nil
      case "case" =>
        Row(s"Function: case") ::
 ...
 */
case class DropFunction(databaseName: Option[String],
                        functionName: String,
                        ifExists: Boolean,
                        isTemp: Boolean)
    extends RunnableCommand {
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (isTemp) {
      ...
      catalog.dropFunction(
          FunctionIdentifier(functionName, databaseName), ignoreIfNotExists = ifExists)
    }
    Seq.empty[Row]
  }
}
...
case class ShowFunctions(db: Option[String], pattern: Option[String]) extends RunnableCommand {
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val functionNames = sparkSession.sessionState.catalog
