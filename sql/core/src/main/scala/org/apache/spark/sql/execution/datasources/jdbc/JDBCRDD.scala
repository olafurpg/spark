      ...
      try {
        ...
        try {
          ...
          while (i < ncols) {
            ...
            val metadata =
              new MetadataBuilder().putString("name", columnName).putLong("scale", fieldScale)
...
private[sql] class JDBCRDD(sc: SparkContext,
   ...
   */
  private val filterWhereClause: String = filters.flatMap(JDBCRDD.compileFilter).mkString(" AND ")
              ...
              case ArrayConversion(elementConversion) =>
                ...
                if (array != null) {
                  val data = elementConversion match {
                    ...
                    case StringConversion =>
                      array.asInstanceOf[Array[java.lang.String]].map(UTF8String.fromString)
                    case DateConversion =>
