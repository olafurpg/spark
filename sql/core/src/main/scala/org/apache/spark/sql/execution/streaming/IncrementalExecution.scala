 ...
 */
class IncrementalExecution(sparkSession: SparkSession,
  ...
  override def planner: SparkPlanner =
    new SparkPlanner(sparkSession.sparkContext, sparkSession.sessionState.conf, stateStrategy)
  ...
  val state = new Rule[SparkPlan] {
    override def apply(plan: SparkPlan): SparkPlan = plan transform {
      case StateStoreSaveExec(
          keys, None, UnaryExecNode(agg, StateStoreRestoreExec(keys2, None, child))) =>
        ...
        StateStoreSaveExec(
            keys,
            Some(stateId),
            agg.withNewChildren(StateStoreRestoreExec(keys, Some(stateId), child) :: Nil))
    }
  }
