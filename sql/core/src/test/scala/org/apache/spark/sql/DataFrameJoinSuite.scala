...
class DataFrameJoinSuite extends QueryTest with SharedSQLContext {
  ...
  test("join - join using") {
    ...
    checkAnswer(
        df.join(df2, "int"), Row(1, "1", "2") :: Row(2, "2", "3") :: Row(3, "3", "4") :: Nil)
  }
  ...
  test("join - join using multiple columns and specifying join type") {
    ...
    checkAnswer(
        df.join(df2, Seq("int", "str"), "left"), Row(1, "1", 2, 3) :: Row(3, "3", 4, null) :: Nil)
  ...
  test("join - join using self join") {
    ...
    // self join
    checkAnswer(
        df.join(df, "int"), Row(1, "1", "1") :: Row(2, "2", "2") :: Row(3, "3", "3") :: Nil)
  }
  ...
  test("join - self join") {
    ...
    checkAnswer(df1.join(df2, $"df1.key" === $"df2.key"),
  ...
  test("join - using aliases after self join") {
    val df = Seq(1, 2, 3).map(i => (i, i.toString)).toDF("int", "str")
    checkAnswer(df.as('x).join(df.as('y), $"x.str" === $"y.str").groupBy("x.str").count(),
                Row("1", 1) :: Row("2", 1) :: Row("3", 1) :: Nil)

    checkAnswer(df.as('x).join(df.as('y), $"x.str" === $"y.str").groupBy("y.str").count(),
                Row("1", 1) :: Row("2", 1) :: Row("3", 1) :: Nil)
  }
  ...
  test("[SPARK-6231] join - self join auto resolve ambiguity") {
    ...
    checkAnswer(
        df.join(df, df("key") === df("key")), Row(1, "1", 1, "1") :: Row(2, "2", 2, "2") :: Nil)

    checkAnswer(
        df.join(df.filter($"value" === "2"), df("key") === df("key")), Row(2, "2", 2, "2") :: Nil)

    checkAnswer(
        df.join(df, df("key") === df("key") && df("value") === 1), Row(1, "1", 1, "1") :: Nil)

    val left = df.groupBy("key").agg(count("*"))
    val right = df.groupBy("key").agg(sum("key"))
    checkAnswer(
        left.join(right, left("key") === right("key")), Row(1, 1, 1, 1) :: Row(2, 1, 2, 2) :: Nil)
  }
