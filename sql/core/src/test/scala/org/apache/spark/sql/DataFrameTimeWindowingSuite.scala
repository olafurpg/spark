...
class DataFrameTimeWindowingSuite extends QueryTest with SharedSQLContext with BeforeAndAfterEach {
  ...
  test("tumbling window groupBy statement") {
    val df = Seq(("2016-03-27 19:39:34", 1, "a"),
  ...
  test("tumbling window groupBy statement with startTime") {
    ...
    checkAnswer(df.groupBy(window($"time", "10 seconds", "10 seconds", "5 seconds"), $"id")
                  .agg(count("*").as("counts"))
                  .orderBy($"window.start".asc)
                  .select("counts"),
                Seq(Row(1), Row(1), Row(1)))
  ...
  test("sliding window grouping") {
    ...
    checkAnswer(
        ...
        Seq(Row("2016-03-27 19:39:18", "2016-03-27 19:39:28", 1),
  ...
  test("windowing combined with explode expression") {
    val df = Seq(("2016-03-27 19:39:34", 1, Seq("a", "b")),
  ...
  test("null timestamps") {
    ...
    checkDataset(df.select(window($"time", "10 seconds"), $"value")
                   .orderBy($"window.start".asc)
                   .select("value")
                   .as[Int],
                 1,
                 2) // null columns are dropped
  }
  ...
  test("time window joins") {
    ...
    checkAnswer(df.select(window($"time", "10 seconds"), $"value")
                  .join(df2.select(window($"time", "10 seconds"), $"othervalue"), Seq("window"))
                  .groupBy("window")
  ...
  test("negative timestamps") {
    val df4 = Seq(("1970-01-01 00:00:02", 1), ("1970-01-01 00:00:12", 2)).toDF("time", "value")
    checkAnswer(
        df4
          .select(window($"time", "10 seconds", "10 seconds", "5 seconds"), $"value")
          .orderBy($"window.start".asc)
          .select($"window.start".cast(StringType), $"window.end".cast(StringType), $"value"),
        Seq(Row("1969-12-31 23:59:55", "1970-01-01 00:00:05", 1),
  ...
  test("millisecond precision sliding windows") {
    val df = Seq(("2016-03-27 09:00:00.41", 3),
