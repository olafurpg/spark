...
class DataFrameWindowSuite extends QueryTest with SharedSQLContext {
  ...
  test("lead") {
    ...
    checkAnswer(df.select(lead("value", 1).over(Window.partitionBy($"key").orderBy($"value"))),
  ...
  test("lag") {
    ...
    checkAnswer(df.select(lag("value", 1).over(Window.partitionBy($"key").orderBy($"value"))),
                Row(null) :: Row("1") :: Row(null) :: Row("2") :: Nil)
  }

  test("lead with default value") {
    val df = Seq((1, "1"), (1, "1"), (2, "2"), (1, "1"), (2, "2"), (1, "1"), (2, "2"))
      .toDF("key", "value")
    df.registerTempTable("window_table")
    checkAnswer(
        df.select(lead("value", 2, "n/a").over(Window.partitionBy("key").orderBy("value"))),
  ...
  test("lag with default value") {
    ...
    df.registerTempTable("window_table")
    checkAnswer(
        df.select(lag("value", 2, "n/a").over(Window.partitionBy($"key").orderBy($"value"))),
  ...
  test("aggregation and rows between with unbounded") {
    ...
    checkAnswer(
        df.select(
            $"key",
            last("key")
              .over(Window.partitionBy($"value").orderBy($"key").rowsBetween(0, Long.MaxValue)),
            last("key")
              .over(Window.partitionBy($"value").orderBy($"key").rowsBetween(Long.MinValue, 0)),
  ...
  test("reverse sliding range frame") {
    ...
    checkAnswer(df.select($"id", avg($"revenue").over(window).cast("int")),
  ...
  test("window function with aggregates") {
    ...
    checkAnswer(
        df.groupBy($"key").agg(sum($"value"), sum(sum($"value")).over(window) - sum($"value")),
        Seq(Row("a", 6, 9), Row("b", 9, 6)))
  }

  test("window function with udaf") {
    val udaf = new UserDefinedAggregateFunction {
      def inputSchema: StructType = new StructType().add("a", LongType).add("b", LongType)
  ...
  test("SPARK-12989 ExtractWindowExpressions treats alias as regular attribute") {
    val src =
      Seq((0, 3, 5)).toDF("a", "b", "c").withColumn("Data", struct("a", "b")).drop("a").drop("b")
  ...
  test("aggregation and rows between with unbounded + predicate pushdown") {
    ...
    checkAnswer(df.select(selectList: _*).where($"value" < "3"),
  ...
  test("aggregation and range between with unbounded + predicate pushdown") {
    ...
    checkAnswer(df.select(selectList: _*).where($"value" < 2),
