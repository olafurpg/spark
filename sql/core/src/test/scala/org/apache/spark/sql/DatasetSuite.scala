...
class DatasetSuite extends QueryTest with SharedSQLContext {
  ...
  test("map with type change with the exact matched number of attributes") {
    ...
    checkDataset(ds.map(identity[(String, Int)]).as[OtherTuple].map(identity[OtherTuple]),
                 OtherTuple("a", 1),
                 OtherTuple("b", 2),
                 OtherTuple("c", 3))
  }
  ...
  test("select 2") {
    val ds = Seq(("a", 1), ("b", 2), ("c", 3)).toDS()
    checkDataset(ds.select(expr("_1").as[String], expr("_2").as[Int]): Dataset[(String, Int)],
                 ("a", 1),
                 ("b", 2),
                 ("c", 3))
  }
  ...
  test("select 2, primitive and tuple") {
    val ds = Seq(("a", 1), ("b", 2), ("c", 3)).toDS()
    checkDataset(ds.select(expr("_1").as[String], expr("struct(_2, _2)").as[(Int, Int)]),
                 ("a", (1, 1)),
                 ("b", (2, 2)),
                 ("c", (3, 3)))
  }
  ...
  test("joinWith, flat schema") {
    ...
    checkDataset(ds1.joinWith(ds2, $"a.value" === $"b.value", "inner"), (1, 1), (2, 2))
  }
  ...
  test("joinWith, expression condition, outer join") {
    ...
    checkDataset(ds1.joinWith(ds2, $"_1" === $"a", "outer"),
  ...
  test("joinWith tuple with primitive, expression") {
    ...
    checkDataset(
        ds1.joinWith(ds2, $"value" === $"_2"), (1, ("a", 1)), (1, ("a", 1)), (2, ("b", 2)))
  }
  ...
  test("joinWith class with primitive, toDF") {
    ...
    checkAnswer(ds1.joinWith(ds2, $"value" === $"b").toDF().select($"_1", $"_2.a", $"_2.b"),
  ...
  test("typed aggregation: expr") {
    ...
    checkDataset(ds.groupByKey(_._1).agg(sum("_2").as[Long]), ("a", 30L), ("b", 3L), ("c", 1L))
  }

  test("typed aggregation: expr, expr") {
    val ds = Seq(("a", 10), ("a", 20), ("b", 1), ("b", 2), ("c", 1)).toDS()

    checkDataset(ds.groupByKey(_._1).agg(sum("_2").as[Long], sum($"_2" + 1).as[Long]),
                 ("a", 30L, 32L),
                 ("b", 3L, 5L),
                 ("c", 1L, 2L))
  }
  ...
  test("typed aggregation: expr, expr, expr") {
    ...
    checkDataset(ds.groupByKey(_._1).agg(sum("_2").as[Long], sum($"_2" + 1).as[Long], count("*")),
                 ("a", 30L, 32L, 2L),
                 ("b", 3L, 5L, 2L),
                 ("c", 1L, 2L, 1L))
  }
  ...
  test("cogroup") {
    ...
    checkDataset(cogrouped, 1 -> "a#", 2 -> "#q", 3 -> "abcfoo#w", 5 -> "hello#er")
  }
  ...
  test("sample without replacement") {
    val n = 100
    val data = sparkContext.parallelize(1 to n, 2).toDS()
    checkDataset(data.sample(withReplacement = false, 0.05, seed = 13), 3, 17, 27, 58, 62)
  }
  ...
  test("SPARK-11894: Incorrect results are returned when using null") {
    ...
    checkDataset(ds1.joinWith(ds2, lit(true)),
                 ((nullInt, "1"), (nullInt, "1")),
