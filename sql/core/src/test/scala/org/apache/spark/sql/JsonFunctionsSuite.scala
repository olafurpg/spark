...
class JsonFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("function get_json_object") {
    ...
    checkAnswer(df.selectExpr("get_json_object(a, '$.name')", "get_json_object(a, '$.age')"),
                Row("alice", "5"))
  }
  ...
  test("json_tuple select") {
    ...
    checkAnswer(df.select($"key", functions.json_tuple($"jstring", "f1", "f2", "f3", "f4", "f5")),
                expected)

    checkAnswer(
        df.selectExpr("key", "json_tuple(jstring, 'f1', 'f2', 'f3', 'f4', 'f5')"), expected)
  }
