...
class MathExpressionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("ceil and ceiling") {
    ...
    checkAnswer(sql("SELECT ceiling(0), ceiling(1), ceiling(1.5)"), Row(0L, 1L, 2L))
  }
  ...
  test("conv") {
    ...
    checkAnswer(df.selectExpr("""conv("9223372036854775807", 36, -16)"""), Row("-1")) // for overflow
  }
  ...
  test("abs") {
    ...
    checkAnswer(input.toDF("key", "value").select(abs($"key").alias("a")).sort("a"),
