...
class SQLQuerySuite extends QueryTest with SharedSQLContext {
  ...
  test("show functions") {
    def getFunctions(pattern: String): Seq[Row] = {
      StringUtils
        .filterPattern(spark.sessionState.functionRegistry.listFunction(), pattern)
        .map(Row(_))
    }
  ...
  test("SPARK-6743: no columns from cache") {
    ...
    checkAnswer(
        sql("SELECT t1.b FROM cachedData, cachedData t1 GROUP BY t1.b"), Row(0) :: Row(81) :: Nil)
  }
  ...
  test("SPARK-8668 expr function") {
    checkAnswer(
        Seq((1, "Bobby G.")).toDF("id", "name").select(expr("length(name)"), expr("abs(id)")),
        Row(8, 1))
  ...
  test("grouping on nested fields") {
    spark.read
      .json(sparkContext.parallelize("""{"nested": {"attribute": 1}, "value": 2}""" :: Nil))
  ...
  test("SPARK-6201 IN type conversion") {
    spark.read
      .json(sparkContext.parallelize(Seq("{\"a\": \"1\"}}", "{\"a\": \"2\"}}", "{\"a\": \"3\"}}")))
      .registerTempTable("d")
  ...
  test("aggregation with codegen") {
    ...
    try {
      ...
      testCodeGen("SELECT value, count(distinct key) FROM testData3x GROUP BY value",
      ...
      testCodeGen(
          "SELECT  sum('a'), avg('a'), count(null) FROM testData", Row(null, null, 0) :: Nil)
    } finally {
  ...
  test("SPARK-3173 Timestamp support in the parser") {
    ...
    checkAnswer(sql("SELECT time FROM timestamps WHERE time='1969-12-31 16:00:00.0'"),
    ...
    checkAnswer(sql("SELECT time FROM timestamps WHERE time='1969-12-31 16:00:00.001'"),
                Row(java.sql.Timestamp.valueOf("1969-12-31 16:00:00.001")))

    checkAnswer(sql("SELECT time FROM timestamps WHERE '1969-12-31 16:00:00.001'=time"),
                Row(java.sql.Timestamp.valueOf("1969-12-31 16:00:00.001")))

    checkAnswer(sql("""SELECT time FROM timestamps WHERE time<'1969-12-31 16:00:00.003'
  ...
  test("index into array of arrays") {
    checkAnswer(
        ...
        arrayData
          .map(d => Row(d.nestedData, d.nestedData(0)(0), d.nestedData(0)(0) + d.nestedData(0)(1)))
          .collect()
          .toSeq)
  }
  ...
  test("agg") {
    checkAnswer(
        sql("SELECT a, SUM(b) FROM testData2 GROUP BY a"), Seq(Row(1, 3), Row(2, 3), Row(3, 3)))
  }

  test("Group By Ordinal - basic") {
    checkAnswer(sql("SELECT a, sum(b) FROM testData2 GROUP BY 1"),
    ...
    checkAnswer(sql("SELECT a, 1, sum(b) FROM testData2 GROUP BY 1, 2"),
                sql("SELECT a, 1, sum(b) FROM testData2 GROUP BY a"))
  }

  test("Group By Ordinal - non aggregate expressions") {
    checkAnswer(sql("SELECT a, b + 2, count(2) FROM testData2 GROUP BY a, 2"),
                sql("SELECT a, b + 2, count(2) FROM testData2 GROUP BY a, b + 2"))

    checkAnswer(sql("SELECT a, b + 2 as c, count(2) FROM testData2 GROUP BY a, 2"),
                sql("SELECT a, b + 2, count(2) FROM testData2 GROUP BY a, b + 2"))
  }

  test("Group By Ordinal - non-foldable constant expression") {
    checkAnswer(sql("SELECT a, b, sum(b) FROM testData2 GROUP BY a, b, 1 + 0"),
  ...
  test("Group By Ordinal - alias") {
    ...
    checkAnswer(sql("SELECT a as b, b as a, sum(b) FROM testData2 GROUP BY 1, 2"),
                sql("SELECT a, b, sum(b) FROM testData2 GROUP BY a, b"))
  }
  ...
  test("Group By Ordinal - negative cases") {
    ...
    var e =
      intercept[UnresolvedException[Aggregate]](sql("SELECT SUM(a) FROM testData2 GROUP BY 1"))
    assert(e.getMessage contains
        "Invalid call to Group by position: the '1'th column in the select contains " +
        "an aggregate function")

    e =
      intercept[UnresolvedException[Aggregate]](sql("SELECT SUM(a) + 1 FROM testData2 GROUP BY 1"))
    assert(e.getMessage contains
        "Invalid call to Group by position: the '1'th column in the select contains " +
        "an aggregate function")

    var ae =
      intercept[AnalysisException](sql("SELECT a, rand(0), sum(b) FROM testData2 GROUP BY a, 2"))
    assert(ae.getMessage contains
        "nondeterministic expression rand(0) should not appear in grouping expression")

    ae = intercept[AnalysisException](sql("SELECT * FROM testData2 GROUP BY a, b, 1"))
    assert(ae.getMessage contains
  ...
  test("Group By Ordinal: spark.sql.groupByOrdinal=false") {
    withSQLConf(SQLConf.GROUP_BY_ORDINAL.key -> "false") {
      ...
      checkAnswer(sql("SELECT * FROM testData2 GROUP BY a, b, 1"),
  ...
  def sortTest(): Unit = {
    checkAnswer(sql("SELECT * FROM testData2 ORDER BY a ASC, b ASC"),
                Seq(Row(1, 1), Row(1, 2), Row(2, 1), Row(2, 2), Row(3, 1), Row(3, 2)))

    checkAnswer(sql("SELECT * FROM testData2 ORDER BY a ASC, b DESC"),
                Seq(Row(1, 2), Row(1, 1), Row(2, 2), Row(2, 1), Row(3, 2), Row(3, 1)))

    checkAnswer(sql("SELECT * FROM testData2 ORDER BY a DESC, b DESC"),
                Seq(Row(3, 2), Row(3, 1), Row(2, 2), Row(2, 1), Row(1, 2), Row(1, 1)))

    checkAnswer(sql("SELECT * FROM testData2 ORDER BY a DESC, b ASC"),
                Seq(Row(3, 1), Row(3, 2), Row(2, 1), Row(2, 2), Row(1, 1), Row(1, 2)))

    checkAnswer(sql("SELECT b FROM binaryData ORDER BY a ASC"), (1 to 5).map(Row(_)))

    checkAnswer(
        sql("SELECT b FROM binaryData ORDER BY a DESC"), (1 to 5).map(Row(_)).toSeq.reverse)

    checkAnswer(sql("SELECT * FROM arrayData ORDER BY data[0] ASC"),
                arrayData.collect().sortBy(_.data(0)).map(Row.fromTuple).toSeq)

    checkAnswer(sql("SELECT * FROM arrayData ORDER BY data[0] DESC"),
    ...
    checkAnswer(sql("SELECT * FROM mapData ORDER BY data[1] DESC"),
  ...
  test("limit") {
    ...
    checkAnswer(
        sql("SELECT * FROM mapData LIMIT 1"), mapData.collect().take(1).map(Row.fromTuple).toSeq)
  }

  test("CTE feature") {
    checkAnswer(sql("with q1 as (select * from testData limit 10) select * from q1"),
  ...
  test("date row") {
    checkAnswer(sql("""select cast("2015-01-28" as date) from testData limit 1"""),
  ...
  test("inner join where, one match per row") {
    withSQLConf(SQLConf.CASE_SENSITIVE.key -> "true") {
      checkAnswer(sql("SELECT * FROM uppercasedata JOIN lowercasedata WHERE n = N"),
                  Seq(Row(1, "A", 1, "a"),
                      Row(2, "B", 2, "b"),
                      Row(3, "C", 3, "c"),
                      Row(4, "D", 4, "d")))
  ...
  test("UNION") {
    ...
    checkAnswer(sql("SELECT * FROM lowerCaseData UNION SELECT * FROM lowerCaseData"),
  ...
  test("EXCEPT") {
    checkAnswer(sql("SELECT * FROM lowerCaseData EXCEPT SELECT * FROM upperCaseData"),
                Row(1, "a") ::
                Row(2, "b") ::
                Row(3, "c") ::
                Row(4, "d") :: Nil)
  ...
  test("INTERSECT") {
    checkAnswer(sql("SELECT * FROM lowerCaseData INTERSECT SELECT * FROM lowerCaseData"),
                Row(1, "a") ::
                Row(2, "b") ::
                Row(3, "c") ::
                Row(4, "d") :: Nil)
  ...
  test("cast boolean to string") {
    ...
    checkAnswer(sql("SELECT CAST(TRUE AS STRING), CAST(FALSE AS STRING) FROM testData LIMIT 1"),
                Row("true", "false"))
  }
  ...
  test("metadata is propagated correctly") {
    ...
    val schemaWithMeta =
      new StructType(Array(schema("id"), schema("name").copy(metadata = metadata), schema("age")))
  ...
  test("SPARK-3371 Renaming a function expression with group by gives error") {
    ...
    checkAnswer(
        sql("SELECT len(value) as temp FROM testData WHERE key = 1 group by len(value)"), Row(1))
  }
  ...
  test("SPARK-3483 Special chars in column names") {
    val data =
      sparkContext.parallelize(Seq("""{"key?number1": "value1", "key.number2": "value2"}"""))
  ...
  test("SPARK-4322 Grouping field with struct field as sub expression") {
    ...
    spark.read.json(sparkContext.makeRDD("""{"a": {"b": 1}}""" :: Nil)).registerTempTable("data")
  ...
  test("SPARK-6145: ORDER BY test for nested fields") {
    spark.read
      .json(sparkContext.makeRDD("""{"a": {"b": 1, "a": {"a": 1}}, "c": [{"d": 1}]}""" :: Nil))
  ...
  test("SPARK-7067: order by queries for complex ExtractValue chain") {
    withTempTable("t") {
      spark.read
        .json(sparkContext.makeRDD("""{"a": {"b": [{"c": 1}]}, "b": [{"d": 1}]}""" :: Nil))
        .registerTempTable("t")
  ...
  test("decimal precision with multiply/division") {
    ...
    checkAnswer(sql("select 10.300000000000000000 * 3.0000000000000000000"), Row(null))
  ...
  test("SPARK-10130 type coercion for IF should have children resolved first") {
    withTempTable("src") {
      ...
      checkAnswer(sql("SELECT IF(a > 0, a, 0) FROM (SELECT key a FROM src) temp"),
                  Seq(Row(1), Row(0)))
    }
  }
  ...
  test("Struct Star Expansion") {
    ...
    checkAnswer(structDf.select($"record.a", $"record.b"),
                Row(1, 1) :: Row(1, 2) :: Row(2, 1) :: Row(2, 2) :: Row(3, 1) :: Row(3, 2) :: Nil)

    checkAnswer(structDf.select($"record.*"),
                Row(1, 1) :: Row(1, 2) :: Row(2, 1) :: Row(2, 2) :: Row(3, 1) :: Row(3, 2) :: Nil)
    ...
    checkAnswer(sql("select struct(a, b) as r1, struct(b, a) as r2 from testData2").select(
                    $"r1.*", $"r2.*"),
    ...
    withTempTable("nestedStructTable") {
      ...
      assert(intercept[AnalysisException](sql("SELECT abc.* FROM nestedStructTable")).getMessage
    ...
    withTempTable("specialCharacterTable") {
      ...
      checkAnswer(specialCharacterPath.select($"`r&&b.c`.*"), nestedStructData.select($"record.*"))
    ...
    assert(intercept[AnalysisException](sql("select a.* from testData2")).getMessage
  ...
  test("Star Expansion - group by") {
    withSQLConf("spark.sql.retainGroupColumns" -> "false") {
      checkAnswer(testData2.groupBy($"a", $"b").agg($"*"),
                  sql("SELECT * FROM testData2 group by a, b"))
    }
  }
  ...
  test("grouping sets when aggregate functions containing groupBy columns") {
    checkAnswer(
        sql("select course, sum(earnings) as sum from courseSales group by course, earnings " +
            "grouping sets((), (course), (course, earnings)) " + "order by course, sum"),
        Row(null, 113000.0) ::
        Row("Java", 20000.0) ::
        Row("Java", 30000.0) ::
        Row("Java", 50000.0) ::
  ...
  test("filter on a grouping column that is not presented in SELECT") {
    checkAnswer(
        sql("select count(1) from (select 1 as a) t group by a having a > 0"), Row(1) :: Nil)
  }
  ...
  test("order by ordinal number") {
    ...
    checkAnswer(sql("SELECT * FROM testData2 ORDER BY 1 ASC, b ASC"),
                Seq(Row(1, 1), Row(1, 2), Row(2, 1), Row(2, 2), Row(3, 1), Row(3, 2)))
  }
  ...
  test("natural join") {
    ...
    withTempTable("nt1", "nt2") {
      ...
      checkAnswer(sql("SELECT * FROM nt1 natural left join nt2 order by v1, v2"),
                  Row("one", 1, 1) :: Row("one", 1, 5) :: Row("two", 2, 22) :: Row("three",
                                                                                   3,
                                                                                   null) :: Nil)

      checkAnswer(sql("SELECT * FROM nt1 natural right join nt2 order by v1, v2"),
                  Row("one", 1, 1) :: Row("one", 1, 5) :: Row("two", 2, 22) :: Nil)

      checkAnswer(sql("SELECT count(*) FROM nt1 natural full outer join nt2"), Row(4) :: Nil)
    }
  }
  ...
  test("join with using clause") {
    ...
    withTempTable("t1", "t2", "t3") {
      ...
      checkAnswer(sql("SELECT * FROM t1 full outer join t3 using (c1)"),
  ...
  test("data source table created in InMemoryCatalog should be able to read/write") {
    withTable("tbl") {
      ...
      checkAnswer(sql("SELECT i, j FROM tbl"),
                  Row(1, "a") :: Row(2, "b") :: Row(3, "c") :: Row(4, "d") :: Nil)
    }
  }
}
