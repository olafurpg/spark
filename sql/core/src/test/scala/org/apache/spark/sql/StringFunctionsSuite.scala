...
class StringFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("string regex_replace / regex_extract") {
    ...
    checkAnswer(
        df.select(regexp_replace($"a", "(\\d+)", "num"), regexp_extract($"a", "(\\d+)-(\\d+)", 1)),
  ...
  test("string trim functions") {
    ...
    checkAnswer(
        df.select(ltrim($"a"), rtrim($"a"), trim($"a")), Row("example  ", "  example", "example"))
  ...
  test("string padding functions") {
    ...
    checkAnswer(
        df.select(
            lpad($"a", 1, "c"), lpad($"a", 5, "??"), rpad($"a", 1, "c"), rpad($"a", 5, "??")),
        Row("h", "???hi", "h", "hi???"))

    checkAnswer(df.selectExpr("lpad(a, b, c)", "rpad(a, b, c)", "lpad(a, 1, c)", "rpad(a, 1, c)"),
                Row("???hi", "hi???", "h", "h"))
  }
  ...
  test("initcap function") {
    ...
    checkAnswer(df.selectExpr("InitCap(x)", "InitCap(y)", "InitCap(z)"), Row("Ab", "A B", "Spark"))
  }
  ...
  test("number format function") {
    ...
    checkAnswer(df.select(format_number(lit(1.toByte), 4)), // convert the 1st argument to integer
                Row("1.0000"))

    checkAnswer(df.select(format_number(lit(2.toShort), 4)), // convert the 1st argument to integer
                Row("2.0000"))
    ...
    checkAnswer(df.select(format_number(lit(6.48173), 4)), // not convert anything
                Row("6.4817"))

    checkAnswer(df.select(format_number(lit(BigDecimal(7.128381)), 4)), // not convert anything
                Row("7.1284"))
