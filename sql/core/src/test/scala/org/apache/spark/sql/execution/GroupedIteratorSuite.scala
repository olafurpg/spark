...
class GroupedIteratorSuite extends SparkFunSuite {
  ...
  test("basic") {
    ...
    val grouped =
      GroupedIterator(input.iterator.map(encoder.toRow), Seq('i.int.at(0)), schema.toAttributes)
  ...
  test("group by 2 columns") {
    ...
    val grouped = GroupedIterator(
        input.iterator.map(encoder.toRow), Seq('i.int.at(0), 'l.long.at(1)), schema.toAttributes)
