...
class PlannerSuite extends SharedSQLContext {
  ...
  test("EnsureRequirements eliminates Exchange if child has Exchange with same partitioning") {
    ...
    val inputPlan = ShuffleExchange(
        finalPartitioning,
        DummySparkPlan(children = DummySparkPlan(outputPartitioning = childPartitioning) :: Nil,
  ...
  test("Reuse exchanges") {
    ...
    val inputPlan = SortMergeJoinExec(Literal(1) :: Nil,
                                      Literal(1) :: Nil,
                                      Inner,
                                      None,
                                      shuffle,
    ...
    // nested exchanges
    val inputPlan2 = SortMergeJoinExec(Literal(1) :: Nil,
                                       Literal(1) :: Nil,
                                       Inner,
                                       None,
