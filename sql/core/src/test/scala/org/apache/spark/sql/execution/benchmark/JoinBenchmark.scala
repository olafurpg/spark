...
class JoinBenchmark extends BenchmarkBase {
  ...
  ignore("broadcast hash join, two int key") {
    val N = 20 << 20
    val M = 1 << 16
    val dim2 = broadcast(sparkSession
          .range(M)
  ...
  ignore("broadcast hash join, two long key") {
    ...
    val M = 1 << 16
    val dim3 = broadcast(
        sparkSession.range(M).selectExpr("id as k1", "id as k2", "cast(id as string) as v"))

    runBenchmark("Join w 2 longs", N) {
      sparkSession
        .range(N)
        .join(dim3, (col("id") % M) === col("k1") && (col("id") % M) === col("k2"))
        .count()
    }
  ...
  ignore("broadcast hash join, two long key with duplicates") {
    ...
    val M = 1 << 16
    val dim4 = broadcast(
        sparkSession.range(M).selectExpr("cast(id/10 as long) as k1", "cast(id/10 as long) as k2"))
  ...
  ignore("sort merge join with duplicates") {
    ...
    runBenchmark("sort merge join", N) {
      val df1 = sparkSession.range(N).selectExpr(s"(id * 15485863) % ${N * 10} as k1")
      val df2 = sparkSession.range(N).selectExpr(s"(id * 15485867) % ${N * 10} as k2")
      df1.join(df2, col("k1") === col("k2")).count()
    }
