...
class CSVSuite extends QueryTest with SharedSQLContext with SQLTestUtils {
  ...
  test("simple csv test") {
    val cars = spark.read.format("csv").option("header", "false").load(testFile(carsFile))

    verifyCars(cars, withHeader = false, checkTypes = false)
  }

  test("simple csv test with calling another function to load") {
    val cars = spark.read.option("header", "false").csv(testFile(carsFile))
  ...
  test("test inferring booleans") {
    ...
    val expectedSchema = StructType(List(StructField("bool", BooleanType, nullable = true)))
  ...
  test("test inferring decimals") {
    ...
    val expectedSchema = StructType(
        List(StructField("decimal", DecimalType(20, 0), nullable = true),
  ...
  test("parse unescaped quotes with maxCharsPerColumn") {
    val rows =
      spark.read.format("csv").option("maxCharsPerColumn", "4").load(testFile(unescapedQuotesFile))
  ...
  test("bad encoding name") {
    val exception = intercept[UnsupportedCharsetException] {
      spark.read.format("csv").option("charset", "1-9588-osi").load(testFile(carsFile8859))
    }
  ...
  test("DDL test parsing decimal type") {
    ...
    assert(spark.sql("SELECT makeName FROM carsTable where priceTag > 60000").collect().size === 1)
  }
  ...
  test("save csv") {
    withTempDir { dir =>
      ...
      val cars = spark.read.format("csv").option("header", "true").load(testFile(carsFile))

      cars.coalesce(1).write.option("header", "true").csv(csvDir)

      val carsCopy = spark.read.format("csv").option("header", "true").load(csvDir)
  ...
  test("save csv with quote") {
    withTempDir { dir =>
      ...
      val carsCopy =
        spark.read.format("csv").option("header", "true").option("quote", "\"").load(csvDir)
  ...
  test("inferring schema with commented lines in CSV data") {
    ...
    val expected = Seq(Seq(1, 2, 3, 4, 5.01D, Timestamp.valueOf("2015-08-20 15:57:00")),
  ...
  test("inferring timestamp types via custom date format") {
    val options =
      Map("header" -> "true", "inferSchema" -> "true", "dateFormat" -> "dd/MM/yyyy hh:mm")
    val results =
      spark.read.format("csv").options(options).load(testFile(datesFile)).select("date").collect()

    val dateFormat = new SimpleDateFormat("dd/MM/yyyy hh:mm")
    val expected = Seq(Seq(new Timestamp(dateFormat.parse("26/08/2015 18:00").getTime)),
  ...
  test("load date types via custom date format") {
    ...
    val options =
      Map("header" -> "true", "inferSchema" -> "false", "dateFormat" -> "dd/MM/yyyy hh:mm")
    val results = spark.read
      .format("csv")
      .options(options)
      .schema(customSchema)
    ...
    val dateFormat = new SimpleDateFormat("dd/MM/yyyy hh:mm")
    val expected = Seq(new Date(dateFormat.parse("26/08/2015 18:00").getTime),
  ...
  test("SPARK-13543 Write the output as uncompressed via option()") {
    ...
    withTempDir { dir =>
      ...
      val carsCopy =
        spark.read.format("csv").option("header", "true").options(extraOptions).load(csvDir)
  ...
  test("Schema inference correctly identifies the datatype when data is sparse.") {
    ...
    assert(df.schema.fields.map(field => field.dataType).deep ==
