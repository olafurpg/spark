...
class JsonSuite extends QueryTest with SharedSQLContext with TestJsonData {
  ...
  test("Type promotion") {
    ...
    checkTypePromotion(
        DateTimeUtils.millisToDays(3601000), enforceCorrectType(ISO8601Time1, DateType))
    ...
    checkTypePromotion(
        DateTimeUtils.millisToDays(10801000), enforceCorrectType(ISO8601Time2, DateType))
  }
  ...
  test("Get compatible type") {
    ...
    // DecimalType
    checkDataType(
        DecimalType.SYSTEM_DEFAULT, DecimalType.SYSTEM_DEFAULT, DecimalType.SYSTEM_DEFAULT)
    ...
    checkDataType(StructType(StructField("f1", IntegerType, true) :: Nil),
                  StructType(Nil),
                  StructType(StructField("f1", IntegerType, true) :: Nil))
    checkDataType(StructType(StructField("f1", IntegerType, true) ::
                      StructField("f2", IntegerType, true) :: Nil),
                  StructType(StructField("f1", LongType, true) :: Nil),
                  StructType(StructField("f1", LongType, true) ::
                      StructField("f2", IntegerType, true) :: Nil))
    checkDataType(StructType(StructField("f1", IntegerType, true) :: Nil),
                  StructType(StructField("f2", IntegerType, true) :: Nil),
                  StructType(StructField("f1", IntegerType, true) ::
                      StructField("f2", IntegerType, true) :: Nil))
    checkDataType(StructType(StructField("f1", IntegerType, true) :: Nil),
  ...
  test("Complex field and type inferring") {
    ...
    val expectedSchema = StructType(
        ...
        StructField("arrayOfStruct",
                    ArrayType(StructType(StructField("field1", BooleanType, true) ::
        ...
        StructField("struct",
                    ...
                    true) ::
        StructField("structWithArrayFields",
  ...
  test("Type conflict in array elements") {
    ...
    checkAnswer(
        sql("select * from jsonTable"),
        Row(Seq("1", "1.1", "true", null, "[]", "{}", "[2,3,4]", """{"field":"str"}"""),
  ...
  test("Loading a JSON dataset primitivesAsString returns complex fields as strings") {
    ...
    val expectedSchema = StructType(
        ...
        StructField("arrayOfString", ArrayType(StringType, true), true) ::
        StructField("arrayOfStruct",
                    ArrayType(StructType(StructField("field1", StringType, true) ::
                                  StructField("field2", StringType, true) ::
                                  StructField("field3", StringType, true) :: Nil),
                              true),
                    true) ::
  ...
  test("Find compatible types even if inferred DecimalType is not capable of other IntegralType") {
    val mixedIntegerAndDoubleRecords = sparkContext.parallelize("""{"a": 3, "b": 1.1}""" ::
        s"""{"a": 3.1, "b": 0.${"0" * 38}1}""" :: Nil)
    val jsonDF = spark.read.option("prefersDecimal", "true").json(mixedIntegerAndDoubleRecords)

    // The values in `a` field will be decimals as they fit in decimal. For `b` field,
    // they will be doubles as `1.0E-39D` does not fit.
    val expectedSchema = StructType(StructField("a", DecimalType(21, 1), true) ::
  ...
  test("Infer big integers correctly even when it does not fit in decimal") {
    val jsonDF = spark.read.json(bigIntegerRecords)
    ...
    val expectedSchema = StructType(StructField("a", DoubleType, true) ::
  ...
  test("Infer floating-point values correctly even when it does not fit in decimal") {
    val jsonDF = spark.read.option("prefersDecimal", "true").json(floatingValueRecords)
    ...
    val mergedJsonDF =
      spark.read.option("prefersDecimal", "true").json(floatingValueRecords ++ bigIntegerRecords)

    val expectedMergedSchema = StructType(StructField("a", DoubleType, true) ::
  ...
  test("Applying schemas with MapType") {
    ...
    val innerStruct = StructType(StructField("field1", ArrayType(IntegerType, true), true) ::
  ...
  test("Corrupt records: FAILFAST mode") {
    ...
    val exceptionOne = intercept[SparkException] {
      spark.read.option("mode", "FAILFAST").json(corruptRecords).collect()
    }
    assert(exceptionOne.getMessage.contains("Malformed line in FAILFAST mode: {"))

    val exceptionTwo = intercept[SparkException] {
      spark.read.option("mode", "FAILFAST").schema(schema).json(corruptRecords).collect()
    }
  ...
  test("Corrupt records: DROPMALFORMED mode") {
    ...
    val jsonDFOne = spark.read.option("mode", "DROPMALFORMED").json(corruptRecords)
    checkAnswer(
        jsonDFOne,
        Row("str_a_4", "str_b_4", "str_c_4") :: Nil
    )
    ...
    val jsonDFTwo =
      spark.read.option("mode", "DROPMALFORMED").schema(schemaTwo).json(corruptRecords)
  ...
  test("Corrupt records: PERMISSIVE mode") {
    ...
    withSQLConf(SQLConf.COLUMN_NAME_OF_CORRUPT_RECORD.key -> "_unparsed") {
      withTempTable("jsonTable") {
        ...
        jsonDF.registerTempTable("jsonTable")
        val schema = StructType(StructField("_unparsed", StringType, true) ::
            StructField("a", StringType, true) ::
            StructField("b", StringType, true) ::
            StructField("c", StringType, true) :: Nil)
  ...
  test("SPARK-13953 Rename the corrupt record field via option") {
    val jsonDF = spark.read.option("columnNameOfCorruptRecord", "_malformed").json(corruptRecords)
    val schema = StructType(
  ...
  test("SPARK-4068: nulls in arrays") {
    ...
    val schema = StructType(
        StructField("field1",
                    ...
                    true) ::
        StructField("field2",
                    ArrayType(ArrayType(StructType(StructField("Test", LongType, true) :: Nil),
                                        true),
                              true),
                    true) ::
        StructField("field3",
                    ArrayType(ArrayType(StructType(StructField("Test", StringType, true) :: Nil),
                                        true),
                              true),
                    true) ::
        StructField("field4", ArrayType(ArrayType(ArrayType(LongType, true), true), true), true) :: Nil)
  ...
  test("JSON with Partition") {
    ...
    withTempPath(root => {
      ...
      // root/dt=1/col1=abc
      val p1_col1 =
        makePartition(sparkContext.parallelize(2 to 5).map(i => s"""{"a": 1, "b": "str$i"}"""),
                      d1,
                      "col1",
                      "abc")
      ...
      // root/dt=1/col1=abd
      val p2 =
        makePartition(sparkContext.parallelize(6 to 10).map(i => s"""{"a": 1, "b": "str$i"}"""),
                      d1,
                      "col1",
                      "abd")
  ...
  test("backward compatibility") {
    ...
    val constantValues = Seq("a string in binary".getBytes(StandardCharsets.UTF_8),
                             null,
                             true,
                             1.toByte,
                             2.toShort,
    ...
    val data = Row.fromSeq(Seq("Spark " + spark.sparkContext.version) ++ constantValues) :: Nil
  ...
  test("SPARK-12872 Support to specify the option for compression codec") {
    withTempDir { dir =>
      ...
      jsonDF.coalesce(1).write.format("json").option("compression", "gZiP").save(jsonDir)

      val compressedFiles = new File(jsonDir).listFiles()
      assert(compressedFiles.exists(_.getName.endsWith(".json.gz")))

      val jsonCopy = spark.read.format("json").load(jsonDir)
  ...
  test("SPARK-13543 Write the output as uncompressed via option()") {
    ...
    withTempDir { dir =>
      ...
      val jsonCopy = spark.read.format("json").options(extraOptions).load(jsonDir)
