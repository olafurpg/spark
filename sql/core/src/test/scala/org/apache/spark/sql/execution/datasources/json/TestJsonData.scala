...
private[json] trait TestJsonData {
  ...
  def primitiveFieldAndType: RDD[String] =
    spark.sparkContext.parallelize("""{"string":"this is a simple string.",
          "integer":10,
          "long":21474836470,
  ...
  def complexFieldAndType2: RDD[String] =
    spark.sparkContext.parallelize("""{"arrayOfStruct":[{"field1": true, "field2": "str1"}, {"field1": false}, {"field3": null}],
  ...
  def timestampAsLong: RDD[String] =
    spark.sparkContext.parallelize("""{"ts":1451732645}""" :: Nil)
  ...
  def floatingValueRecords: RDD[String] =
    spark.sparkContext.parallelize(s"""{"a": 0.${"0" * 38}1, "b": 0.01}""" :: Nil)

  def bigIntegerRecords: RDD[String] =
    spark.sparkContext.parallelize(s"""{"a": 1${"0" * 38}, "b": 92233720368547758070}""" :: Nil)
