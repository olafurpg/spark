...
class ExistenceJoinSuite extends SparkPlanTest with SharedSQLContext {
  ...
  private lazy val composedConditionEQ = {
    And((left.col("a") === right.col("c")).expr, LessThan(left.col("b").expr, right.col("d").expr))
  }
  ...
  private def testExistenceJoin(testName: String,
                                joinType: JoinType,
                                leftRows: => DataFrame,
                                rightRows: => DataFrame,
          ...
          withSQLConf(SQLConf.SHUFFLE_PARTITIONS.key -> "1") {
            checkAnswer2(
                ...
                (left: SparkPlan, right: SparkPlan) =>
                  EnsureRequirements(left.sqlContext.sessionState.conf).apply(ShuffledHashJoinExec(
            ...
            checkAnswer2(leftRows,
                         ...
                         (left: SparkPlan, right: SparkPlan) =>
                           EnsureRequirements(left.sqlContext.sessionState.conf).apply(
                               createLeftSemiPlusJoin(ShuffledHashJoinExec(leftKeys,
                                                                           rightKeys,
                                                                           leftSemiPlus,
                                                                           BuildRight,
                                                                           boundCondition,
        ...
        case (_, leftKeys, rightKeys, boundCondition, _, _) =>
          withSQLConf(SQLConf.SHUFFLE_PARTITIONS.key -> "1") {
            ...
            checkAnswer2(leftRows,
                         ...
                         (left: SparkPlan, right: SparkPlan) =>
                           EnsureRequirements(left.sqlContext.sessionState.conf).apply(
                               createLeftSemiPlusJoin(BroadcastHashJoinExec(leftKeys,
                                                                            rightKeys,
                                                                            leftSemiPlus,
                                                                            BuildRight,
                                                                            boundCondition,
        ...
        case (_, leftKeys, rightKeys, boundCondition, _, _) =>
          withSQLConf(SQLConf.SHUFFLE_PARTITIONS.key -> "1") {
            checkAnswer2(
                ...
                rightRows,
                (left: SparkPlan, right: SparkPlan) =>
                  EnsureRequirements(left.sqlContext.sessionState.conf).apply(SortMergeJoinExec(
                          leftKeys, rightKeys, joinType, boundCondition, left, right)),
                expectedAnswer,
                sortAnswers = true)
            checkAnswer2(
                leftRows,
                ...
                (left: SparkPlan, right: SparkPlan) =>
                  EnsureRequirements(left.sqlContext.sessionState.conf)
                    .apply(createLeftSemiPlusJoin(SortMergeJoinExec(
    ...
    test(s"$testName using BroadcastNestedLoopJoin build left") {
      withSQLConf(SQLConf.SHUFFLE_PARTITIONS.key -> "1") {
        ...
        checkAnswer2(leftRows,
                     ...
                     (left: SparkPlan, right: SparkPlan) =>
                       EnsureRequirements(left.sqlContext.sessionState.conf)
                         .apply(createLeftSemiPlusJoin(BroadcastNestedLoopJoinExec(
  ...
  testExistenceJoin("test single condition (equal) for left semi join",
                    LeftSemi,
                    left,
                    right,
                    singleConditionEQ,
  ...
  testExistenceJoin("test composed condition (equal & non-equal) for left semi join",
                    LeftSemi,
                    left,
                    right,
                    composedConditionEQ,
  ...
  testExistenceJoin("test composed condition (both non-equal) for left semi join",
                    LeftSemi,
                    left,
                    right,
  ...
  testExistenceJoin("test single condition (equal) for left Anti join",
                    LeftAnti,
                    left,
                    right,
                    singleConditionEQ,
  ...
  testExistenceJoin("test composed condition (both non-equal) for anti join",
                    LeftAnti,
                    left,
                    right,
                    composedConditionNEQ,
