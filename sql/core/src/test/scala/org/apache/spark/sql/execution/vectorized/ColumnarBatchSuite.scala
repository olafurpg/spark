...
class ColumnarBatchSuite extends SparkFunSuite {
    ...
    (MemoryMode.ON_HEAP :: MemoryMode.OFF_HEAP :: Nil).foreach { memMode =>
      {
        ...
        // Just put llo
        val offset = column.putByteArray(idx,
                                         values(0).getBytes(StandardCharsets.UTF_8),
                                         2,
        ...
        reference.zipWithIndex.foreach { v =>
          ...
          assert(v._1 == column.getUTF8String(v._2).toString, "MemoryMode" + memMode)
        }
    ...
    (MemoryMode.ON_HEAP :: MemoryMode.OFF_HEAP :: Nil).foreach { memMode =>
      {
        ...
        val a1 =
          ColumnVectorUtils.toPrimitiveJavaArray(column.getArray(0)).asInstanceOf[Array[Int]]
        val a2 =
          ColumnVectorUtils.toPrimitiveJavaArray(column.getArray(1)).asInstanceOf[Array[Int]]
        val a3 =
          ColumnVectorUtils.toPrimitiveJavaArray(column.getArray(2)).asInstanceOf[Array[Int]]
        val a4 =
          ColumnVectorUtils.toPrimitiveJavaArray(column.getArray(3)).asInstanceOf[Array[Int]]
        assert(a1 === Array(0))
        assert(a2 === Array(1, 2))
        ...
        assert(
            ColumnVectorUtils.toPrimitiveJavaArray(column.getArray(0)).asInstanceOf[Array[Int]] === array)
      }
    }
  }
  ...
  private def compareStruct(fields: Seq[StructField], r1: InternalRow, r2: Row, seed: Long) {
    fields.zipWithIndex.foreach {
      case (field: StructField, ordinal: Int) =>
        ...
        if (!r1.isNullAt(ordinal)) {
          field.dataType match {
            ...
            case ShortType =>
              assert(r1.getShort(ordinal) == r2.getShort(ordinal), "Seed = " + seed)
            ...
            case StructType(childFields) =>
              compareStruct(
                  childFields, r1.getStruct(ordinal, fields.length), r2.getStruct(ordinal), seed)
            case _ =>
