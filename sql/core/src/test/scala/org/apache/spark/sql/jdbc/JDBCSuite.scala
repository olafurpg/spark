...
class JDBCSuite
  ...
  before {
    ...
    conn
      .prepareStatement("insert into test.people values ('joe ''foo'' \"bar\"', 3)")
      .executeUpdate()
    conn.commit()
    ...
    conn
      .prepareStatement(
          "create table test.inttypes (a INT, b BOOLEAN, c TINYINT, " + "d SMALLINT, e BIGINT)")
      .executeUpdate()
    conn
      .prepareStatement("insert into test.inttypes values (1, false, 3, 4, 1234567890123)")
      .executeUpdate()
    conn
      .prepareStatement("insert into test.inttypes values (null, null, null, null, null)")
      .executeUpdate()
    conn.commit()
    sql(s"""
    ...
    conn
      .prepareStatement("create table test.strtypes (a BINARY(20), b VARCHAR(20), " +
    ...
    conn
      .prepareStatement("create table test.timetypes (a TIME, b DATE, c TIMESTAMP)")
      .executeUpdate()
    conn
    ...
    conn
      .prepareStatement("create table test.flttypes (a DOUBLE, b REAL, c DECIMAL(38, 18))")
      .executeUpdate()
    conn
    ...
    conn
      .prepareStatement(
          "create table test.emp(name TEXT(32) NOT NULL," + " theid INTEGER, \"Dept\" INTEGER)")
      .executeUpdate()
    conn.prepareStatement("insert into test.emp values ('fred', 1, 10)").executeUpdate()
    conn.prepareStatement("insert into test.emp values ('mary', 2, null)").executeUpdate()
    conn
      .prepareStatement("insert into test.emp values ('joe ''foo'' \"bar\"', 3, 30)")
      .executeUpdate()
    conn.prepareStatement("insert into test.emp values ('kathy', null, null)").executeUpdate()
    conn.commit()
  ...
  test("SELECT * WHERE (simple predicates)") {
    ...
    assert(
        checkPushdown(sql("SELECT * FROM foobar WHERE NAME IN ('mary', 'fred')")).collect().size == 2)
    assert(
        checkPushdown(sql("SELECT * FROM foobar WHERE NAME NOT IN ('fred')")).collect().size == 2)
    ...
    assert(
        checkNotPushdown(sql("SELECT * FROM foobar WHERE (THEID + 2) != 4")).collect().size == 2)
  }
  ...
  test("Basic API with FetchSize") {
    ...
    assert(spark.read.jdbc(urlWithUserAndPass, "TEST.PEOPLE", properties).collect().length === 3)
  }
  ...
  test("test DATE types") {
    ...
    val cachedRows =
      spark.read.jdbc(urlWithUserAndPass, "TEST.TIMETYPES", new Properties).cache().collect()
  ...
  test("compile filters") {
    ...
    assert(
        doCompileFilter(LessThan("col3", Timestamp.valueOf("1995-11-21 00:00:00.0"))) === "col3 < '1995-11-21 00:00:00.0'")
    ...
    assert(
        doCompileFilter(And(EqualNullSafe("col0", "abc"), EqualTo("col1", "def"))) === "((NOT (col0 != 'abc' OR col0 IS NULL OR 'abc' IS NULL) " +
  ...
  test("test credentials in the properties are not in plan output") {
    ...
    spark.executePlan(explain).executedPlan.executeCollect().foreach { r =>
