...
class InsertSuite extends DataSourceTest with SharedSQLContext {
  ...
  test("it is not allowed to write to a table while querying it.") {
    ...
    assert(message.contains("Cannot overwrite a path that is also being read from."),
  ...
  test("Caching") {
    ...
    checkAnswer(sql("SELECT x.a, y.a FROM jsonTable x JOIN jsonTable y ON x.a = y.a + 1"),
