...
class PrunedScanSuite extends DataSourceTest with SharedSQLContext {
  ...
  sqlTest("SELECT x.b, y.a FROM oneToTenPruned x JOIN oneToTenPruned y ON x.a = y.b",
          (1 to 5).map(i => Row(i * 4, i)).toSeq)

  sqlTest("SELECT x.a, y.b FROM oneToTenPruned x JOIN oneToTenPruned y ON x.a = y.b",
