...
case class AllDataTypesScan(
    from: Int, to: Int, userSpecifiedSchema: StructType)(@transient val sparkSession: SparkSession)
    extends BaseRelation
    with TableScan {
...
class TableScanSuite extends DataSourceTest with SharedSQLContext {
  ...
  sqlTest("SELECT structFieldComplex.Value.`value_(2)` FROM tableWithSchema",
  ...
  test("Caching") {
    ...
    checkAnswer(sql("SELECT a.i, b.i FROM oneToTen a JOIN oneToTen b ON a.i = b.i + 1"),
                (2 to 10).map(i => Row(i, i - 1)).toSeq)
