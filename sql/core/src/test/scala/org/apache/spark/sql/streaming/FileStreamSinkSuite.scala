...
class FileStreamSinkSuite extends StreamTest with SharedSQLContext {
  ...
  test("FileStreamSinkWriter - unpartitioned data") {
    ...
    def writeRange(start: Int, end: Int, numPartitions: Int): Seq[String] = {
      val df = spark.range(start, end, 1, numPartitions).select($"id", lit(100).as("data"))
  ...
  test("FileStreamSink - partitioned writing and batch reading") {
    ...
    try {
      ...
      val expectedSchema =
        new StructType().add(StructField("value", IntegerType)).add(StructField("id", IntegerType))
      ...
      checkDataset(outputDf.as[(Int, Int)], (1000, 1), (2000, 2), (3000, 3))
  ...
  test("FileStreamSink - supported formats") {
    def testFormat(format: Option[String]): Unit = {
      ...
      try {
        val writer = ds.map(i => (i, i * 1000)).toDF("id", "value").write
        if (format.nonEmpty) {
          writer.format(format.get)
        }
        query = writer.option("checkpointLocation", checkpointDir).startStream(outputDir)
      } finally {
        if (query != null) {
          query.stop()
        }
  ...
  private def checkFilesExist(dir: File, expectedFiles: Seq[String], msg: String): Unit = {
    ...
    val files = FileUtils
      .listFiles(dir, new RegexFileFilter("[^.]+"), DirectoryFileFilter.DIRECTORY)
      .asScala
