...
private[sql] trait SQLTestData { self =>
  ...
  protected lazy val largeAndSmallInts: DataFrame = {
    val df = spark.sparkContext
      .parallelize(LargeAndSmallInts(2147483644, 1) ::
  ...
  protected lazy val binaryData: DataFrame = {
    val df = spark.sparkContext
      .parallelize(BinaryData("12".getBytes(StandardCharsets.UTF_8), 1) ::
  ...
  protected lazy val arrayData: RDD[ArrayData] = {
    val rdd = spark.sparkContext.parallelize(ArrayData(Seq(1, 2, 3), Seq(Seq(1, 2, 3))) ::
  ...
  protected lazy val nullableRepeatedData: RDD[StringData] = {
    val rdd = spark.sparkContext.parallelize(List.fill(2)(StringData(null)) ++
  ...
  protected lazy val complexData: DataFrame = {
    val df = spark.sparkContext
      .parallelize(ComplexData(Map("1" -> 1), TestData(1, "1"), Seq(1, 1, 1), true) ::
  ...
  protected lazy val courseSales: DataFrame = {
    val df = spark.sparkContext
      .parallelize(CourseSales("dotNET", 2012, 10000) ::
