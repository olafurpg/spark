      ...
      if (numStatement > 0) {
        ...
        Some(
            UIUtils.listingTable(
                headerRow, generateDataRow, dataRows, false, None, Seq(null), false))
      } else {
        None
      }
      ...
      if (numBatches > 0) {
        ...
        def generateDataRow(session: SessionInfo): Seq[Node] = {
          val sessionLink = "%s/%s/session?id=%s".format(
              UIUtils.prependBaseUri(parent.basePath), parent.prefix, session.sessionId)
          <tr>
        ...
        Some(
            UIUtils.listingTable(
                headerRow, generateDataRow, dataRows, true, None, Seq(null), false))
      } else {
        None
      }
