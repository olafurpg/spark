...
private[client] sealed abstract class Shim {
  ...
  protected def findStaticMethod(klass: Class[_], name: String, args: Class[_]*): Method = {
    ...
    require(
        Modifier.isStatic(method.getModifiers()), s"Method $name of class $klass is not static.")
    method
  }
...
private[client] class Shim_v0_12 extends Shim with Logging {
  ...
  private lazy val getCommandProcessorMethod = findStaticMethod(
      classOf[CommandProcessorFactory], "get", classOf[String], classOf[HiveConf])
...
private[client] class Shim_v0_13 extends Shim_v0_12 {
  ...
  private lazy val setCurrentSessionStateMethod = findStaticMethod(
      classOf[SessionState], "setCurrentSessionState", classOf[SessionState])
  private lazy val setDataLocationMethod = findMethod(
  ...
  private lazy val getPartitionsByFilterMethod = findMethod(
      classOf[Hive], "getPartitionsByFilter", classOf[Table], classOf[String])
...
private[client] class Shim_v0_14 extends Shim_v0_13 {
  ...
  override def getMetastoreClientConnectRetryDelayMillis(conf: HiveConf): Long = {
    getTimeVarMethod
      .invoke(conf, HiveConf.ConfVars.METASTORE_CLIENT_CONNECT_RETRY_DELAY, TimeUnit.MILLISECONDS)
      .asInstanceOf[Long]
  }
