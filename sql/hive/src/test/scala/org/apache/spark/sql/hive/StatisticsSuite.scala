...
class StatisticsSuite extends QueryTest with TestHiveSingleton {
  ...
  test("parse analyze commands") {
    ...
    assertAnalyzeCommand("ANALYZE TABLE Table1 COMPUTE STATISTICS", classOf[AnalyzeTable])
    ...
    assertAnalyzeCommand(
        ...
        classOf[AnalyzeTable])
    assertAnalyzeCommand(
        "ANALYZE TABLE Table1 PARTITION(ds, hr) COMPUTE STATISTICS", classOf[AnalyzeTable])
    assertAnalyzeCommand(
        "ANALYZE TABLE Table1 PARTITION(ds, hr) COMPUTE STATISTICS noscan", classOf[AnalyzeTable])

    assertAnalyzeCommand("ANALYZE TABLE Table1 COMPUTE STATISTICS nOscAn", classOf[AnalyzeTable])
  }
  ...
  ignore("analyze MetastoreRelations") {
    ...
    hiveContext.sessionState.catalog
      .dropTable(TableIdentifier("tempTable"), ignoreIfNotExists = true)
  }
  ...
  test("auto converts to broadcast left semi join, by size estimate of a relation") {
    val leftSemiJoinQuery = """SELECT * FROM src a
    ...
    val sizes = df.queryExecution.analyzed.collect {
      case r
          if implicitly[ClassTag[MetastoreRelation]].runtimeClass.isAssignableFrom(r.getClass) =>
    ...
    assert(
        bhj.size === 1, s"actual query plans do not contain broadcast join: ${df.queryExecution}")
