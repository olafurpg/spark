...
abstract class AggregationQuerySuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  override def beforeAll(): Unit = {
    ...
    val data2 = Seq[(Integer, Integer, Integer)]((1, 10, -10),
                                                 (null, -60, 60),
                                                 (1, 30, -30),
                                                 (1, 30, 30),
                                                 (2, 1, 1),
  ...
  test("group by function") {
    ...
    checkAnswer(sql("SELECT floor(a) AS a, collect_set(b) FROM data GROUP BY floor(a) ORDER BY a"),
                Row(1, Array(2)) :: Nil)
  }
  ...
  test("pearson correlation") {
    ...
    val covar_tab = Seq[(Integer, Integer, Integer)]((1, null, 15),
                                                     (2, 3, null),
                                                     (3, 7, 12),
                                                     (4, 4, 14),
                                                     (5, 8, 17),
  ...
  test("no aggregation function (SPARK-11486)") {
    val df =
      spark.range(20).selectExpr("id", "repeat(id, 1) as s").groupBy("s").count().groupBy().count()
  ...
  test("udaf with all data types") {
    ...
    Seq(dataTypes, UnsafeRow.mutableFieldTypes.asScala.toSeq).foreach { dataTypes =>
      ...
      // create RDD.
      val maybeDataGenerator = RandomDataGenerator.forType(dataType = schemaForGenerator,
                                                           nullable = true,
  ...
  test("udaf without specifying inputSchema") {
    withTempTable("noInputSchemaUDAF") {
      ...
      val schema = StructType(StructField("key", IntegerType) ::
          StructField("myArray", ArrayType(StructType(StructField("v", IntegerType) :: Nil))) :: Nil)
      spark
