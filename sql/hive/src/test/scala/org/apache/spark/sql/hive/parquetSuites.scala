  ...
  test("SPARK-15248: explicitly added partitions should be readable") {
    withTable("test_added_partitions", "test_temp") {
      withTempDir { src =>
        ...
        checkAnswer(sql("SELECT * FROM test_added_partitions"),
...
class ParquetSourceSuite extends ParquetPartitioningTest {
  ...
  test("SPARK-8811: compatibility with array of struct in Hive") {
    withTempPath { dir =>
      ...
      withTable("array_of_struct") {
        val conf = Seq(HiveUtils.CONVERT_METASTORE_PARQUET.key -> "false",
        ...
        withSQLConf(conf: _*) {
          ...
          checkAnswer(spark.read.parquet(path), Row("1st", "2nd", Seq(Row("val_a", "val_b"))))
        }
      }
    }
  }
  ...
  test("values in arrays and maps stored in parquet are always nullable") {
    ...
    val expectedSchema1 = StructType(StructField("m", mapType1, nullable = true) ::
    ...
    withTable("alwaysNullable") {
      ...
      val expectedSchema2 = StructType(StructField("m", mapType2, nullable = true) ::
          StructField("a", arrayType2, nullable = true) :: Nil)

      assert(table("alwaysNullable").schema === expectedSchema2)

      checkAnswer(sql("SELECT m, a FROM alwaysNullable"), Row(Map(2 -> 3), Seq(4, 5, 6)))
    }
  }
...
abstract class ParquetPartitioningTest extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  Seq("partitioned_parquet",
    ...
    test(s"sum $table") {
      checkAnswer(sql(s"SELECT SUM(intField) FROM $table WHERE intField IN (1,2,3) AND p = 1"),
                  Row(1 + 2 + 3))
    }
  ...
  Seq("partitioned_parquet_with_key_and_complextypes", "partitioned_parquet_with_complextypes").foreach {
    table =>
