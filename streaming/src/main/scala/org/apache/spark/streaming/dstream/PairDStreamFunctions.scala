...
class PairDStreamFunctions[K, V](
    self: DStream[(K, V)])(implicit kt: ClassTag[K], vt: ClassTag[V], ord: Ordering[K])
   ...
   */
  def reduceByKey(reduceFunc: (V, V) => V, numPartitions: Int): DStream[(K, V)] = ssc.withScope {
  ...
  def groupByKeyAndWindow(
      windowDuration: Duration, slideDuration: Duration): DStream[(K, Iterable[V])] =
    ssc.withScope {
  ...
  def reduceByKeyAndWindow(
    ...
    reduceByKeyAndWindow(
        reduceFunc, windowDuration, slideDuration, defaultPartitioner(numPartitions))
  }
  ...
  def updateStateByKey[S: ClassTag](
    ...
    new StateDStream(
        self, ssc.sc.clean(updateFunc), partitioner, rememberPartitioner, Some(initialRDD))
  }
  ...
  def saveAsHadoopFiles[F <: OutputFormat[K, V]](
    ...
    saveAsHadoopFiles(prefix, suffix, keyClass, valueClass, fm.runtimeClass.asInstanceOf[Class[F]])
  }
  ...
  def saveAsHadoopFiles(
    ...
    val saveFunc = (rdd: RDD[(K, V)], time: Time) => {
      ...
      rdd.saveAsHadoopFile(
          file, keyClass, valueClass, outputFormatClass, new JobConf(serializableConf.value))
    }
  ...
  def saveAsNewAPIHadoopFiles[F <: NewOutputFormat[K, V]](
    ...
    saveAsNewAPIHadoopFiles(
        prefix, suffix, keyClass, valueClass, fm.runtimeClass.asInstanceOf[Class[F]])
  }
