...
private[ui] class BatchPage(parent: StreamingTab) extends WebUIPage("batch") {
  ...
  private def generateOutputOpRowWithoutSparkJobs(outputOpData: OutputOperationUIData,
                                                  outputOpDescription: Seq[Node],
  ...
  private def generateOutputOpIdRow(
      outputOpData: OutputOperationUIData, sparkJobs: Seq[SparkJobIdWithUIData]): Seq[Node] = {
  ...
  private def generateJobTable(batchUIData: BatchUIData): Seq[Node] = {
    val outputOpIdToSparkJobIds = batchUIData.outputOpIdSparkJobIdPairs.groupBy(_.outputOpId).map {
