...
private[streaming] class RateLimitedOutputStream(out: OutputStream, desiredBytesPerSec: Int)
  ...
  private def waitToWrite(numBytes: Int) {
    ...
    if (rate < desiredBytesPerSec) {
      ...
      if (sleepTimeInMillis > 0) {
        logTrace(
            "Natural rate is " + rate + " per second but desired rate is " + desiredBytesPerSec +
            ", sleeping for " + sleepTimeInMillis + " ms to compensate.")
