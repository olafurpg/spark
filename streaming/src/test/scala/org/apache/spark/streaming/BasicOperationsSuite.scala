...
class BasicOperationsSuite extends TestSuiteBase {
  ...
  test("updateStateByKey - simple with initial value RDD") {
    ...
    val updateStateOperation = (s: DStream[String]) => {
      ...
      s.map(x => (x, 1))
        .updateStateByKey[Int](updateFunc, new HashPartitioner(numInputPartitions), initialRDD)
    }
    ...
    val updateStateOperation = (s: DStream[String]) => {
      ...
      s.map(x => (x, 1))
        .updateStateByKey[Int](
            newUpdateFunc, new HashPartitioner(numInputPartitions), true, initialRDD)
    }
  ...
  test("rdd cleanup - map and window") {
    ...
    def operation(s: DStream[Int]): DStream[(Int, Int)] = {
      s.map(x => (x % 10, 1)).window(Seconds(2), Seconds(1)).window(Seconds(4), Seconds(2))
    }

    val operatedStream = runCleanupTest(conf,
                                        operation _,
  ...
  test("rdd cleanup - updateStateByKey") {
    ...
    val stateStream =
      runCleanupTest(conf, _.map(_ -> 1).updateStateByKey(updateFunc).checkpoint(Seconds(3)))
