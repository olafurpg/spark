...
class ExecutorAllocationManagerSuite
  ...
  test("basic functionality") {
    ...
    withAllocationManager(numReceivers = 1) {
        ...
        def verifyTotalRequestedExecs(expectedRequestedTotalExecs: Option[Int]): Unit = {
          if (expectedRequestedTotalExecs.nonEmpty) {
            ...
            verify(allocationClient, times(1))
              .requestTotalExecutors(meq(expectedRequestedTotalExecs.get), meq(0), meq(Map.empty))
          } else {
  ...
  test("requestExecutors policy") {
    ...
    /** Verify that the expected number of total executor were requested */
    def verifyRequestedExecs(numExecs: Int, numNewExecs: Int, expectedRequestedTotalExecs: Int)(
      ...
      verify(allocationClient, times(1))
        .requestTotalExecutors(meq(expectedRequestedTotalExecs), meq(0), meq(Map.empty))
    }
    ...
    withAllocationManager(numReceivers = 2) {
      case (_, allocationManager) =>
        implicit val am = allocationManager
  ...
  test("killExecutor policy") {
     ...
     */
    def verifyKilledExec(execIds: Seq[String],
  ...
  test("parameter validation") {
    ...
    def validateParams(numReceivers: Int = 1,
      ...
      if (scalingIntervalSecs.nonEmpty) {
        conf.set(
            "spark.streaming.dynamicAllocation.scalingInterval", s"${scalingIntervalSecs.get}s")
      }
      ...
      if (scalingDownRatio.nonEmpty) {
        conf.set(
            "spark.streaming.dynamicAllocation.scalingDownRatio", scalingDownRatio.get.toString)
      }
  ...
  test("enabling and disabling") {
    ...
    withStreamingContext(new SparkConf().set("spark.streaming.dynamicAllocation.enabled", "true")) {
      ssc =>
        ssc.start()
