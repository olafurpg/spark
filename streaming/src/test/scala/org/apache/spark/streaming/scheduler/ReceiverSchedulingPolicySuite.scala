...
class ReceiverSchedulingPolicySuite extends SparkFunSuite {
  ...
  test("rescheduleReceiver: return all idle executors if there are any idle executors") {
    ...
    val receiverTrackingInfoMap =
      Map(0 -> ReceiverTrackingInfo(0, ReceiverState.ACTIVE, None, Some(executors(0))))
    val scheduledLocations =
      receiverSchedulingPolicy.rescheduleReceiver(1, None, receiverTrackingInfoMap, executors)
  ...
  test("rescheduleReceiver: return all executors that have minimum weight if no idle executors") {
    ...
    val receiverTrackingInfoMap =
      Map(0 -> ReceiverTrackingInfo(
              0, ReceiverState.ACTIVE, None, Some(ExecutorCacheTaskLocation("host1", "1"))),
          1 -> ReceiverTrackingInfo(1,
          ...
          2 -> ReceiverTrackingInfo(2,
                                    ReceiverState.SCHEDULED,
                                    Some(Seq(ExecutorCacheTaskLocation("host1", "1"),
                                             ExecutorCacheTaskLocation("host3", "3"))),
                                    None),
          3 -> ReceiverTrackingInfo(4,
                                    ReceiverState.SCHEDULED,
                                    Some(Seq(ExecutorCacheTaskLocation("host4", "4"),
                                             ExecutorCacheTaskLocation("host5", "5"))),
                                    None))
    val scheduledLocations =
      receiverSchedulingPolicy.rescheduleReceiver(4, None, receiverTrackingInfoMap, executors)
  ...
  test("scheduleReceivers: " +
    ...
    val executors =
      (0 until 3).map(executorId => ExecutorCacheTaskLocation("localhost", executorId.toString))
  ...
  test("scheduleReceivers: " +
    ...
    val executors =
      (0 until 6).map(executorId => ExecutorCacheTaskLocation("localhost", executorId.toString))
    val scheduledLocations = receiverSchedulingPolicy.scheduleReceivers(receivers, executors)
    val numReceiversOnExecutor = mutable.HashMap[TaskLocation, Int]()
  ...
  test("scheduleReceivers: schedule receivers evenly when the preferredLocations are even") {
    val receivers =
      (0 until 3).map(new RateTestReceiver(_)) ++
      (3 until 6).map(new RateTestReceiver(_, Some("localhost")))
    val executors =
      (0 until 3).map(executorId => ExecutorCacheTaskLocation("localhost", executorId.toString)) ++
      (3 until 6).map(executorId => ExecutorCacheTaskLocation("localhost2", executorId.toString))
    val scheduledLocations = receiverSchedulingPolicy.scheduleReceivers(receivers, executors)
    val numReceiversOnExecutor = mutable.HashMap[TaskLocation, Int]()
    ...
    assert(
        ...
        (0 until 3)
          .map(executorId => ExecutorCacheTaskLocation("localhost", executorId.toString))
          .toSet)
  }
