...
class YarnSparkHadoopUtil extends SparkHadoopUtil {
  ...
  def obtainTokenForHiveMetastore(
      sparkConf: SparkConf, conf: Configuration, credentials: Credentials) {
  ...
  def obtainTokenForHBase(
      sparkConf: SparkConf, conf: Configuration, credentials: Credentials): Unit = {
  ...
  private[yarn] def obtainTokenForHiveMetastoreInner(
    ...
    // in the hive config.
    val ctor =
      hiveConfClass.getDeclaredConstructor(classOf[Configuration], classOf[Object].getClass)
    ...
    if (metastoreUri.nonEmpty) {
      ...
      try {
        ...
        val getDelegationToken =
          hiveClass.getMethod("getDelegationToken", classOf[String], classOf[String])
...
object YarnSparkHadoopUtil {
  ...
  def get: YarnSparkHadoopUtil = {
    val yarnMode = java.lang.Boolean
      .parseBoolean(System.getProperty("SPARK_YARN_MODE", System.getenv("SPARK_YARN_MODE")))
    if (!yarnMode) {
  ...
  private lazy val expandMethod =
    Try(classOf[Environment].getMethod("$$")).getOrElse(classOf[Environment].getMethod("$"))
