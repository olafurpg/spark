...
class ClientDistributedCacheManagerSuite extends SparkFunSuite with MockitoSugar {
  ...
  class MockClientDistributedCacheManager extends ClientDistributedCacheManager {
    override def getVisibility(conf: Configuration,
                               uri: URI,
  ...
  test("test getFileStatus cached") {
    ...
    val uri = new URI("/tmp/testing")
    val realFileStatus =
      new FileStatus(10, false, 1, 1024, 10, 10, null, "testOwner", null, new Path("/tmp/testing"))
    when(fs.getFileStatus(new Path(uri))).thenReturn(new FileStatus())
  ...
  test("test addResource") {
    ...
    distMgr.addResource(
        fs, conf, destPath, localResources, LocalResourceType.FILE, "link", statCache, false)
    ...
    assert(
        sparkConf.get(CACHED_FILES_VISIBILITIES) === Seq(LocalResourceVisibility.PRIVATE.name()))
    assert(sparkConf.get(CACHED_FILES_TYPES) === Seq(LocalResourceType.FILE.name()))

    // add another one and verify both there and order correct
    val realFileStatus = new FileStatus(
        20, false, 1, 1024, 10, 30, null, "testOwner", null, new Path("/tmp/testing2"))
    val destPath2 = new Path("file:///foo.invalid.com:8080/tmp/testing2")
    when(fs.getFileStatus(destPath2)).thenReturn(realFileStatus)
    distMgr.addResource(
        fs, conf, destPath2, localResources, LocalResourceType.FILE, "link2", statCache, false)
  ...
  test("test addResource link null") {
    ...
    intercept[Exception] {
      distMgr.addResource(
          fs, conf, destPath, localResources, LocalResourceType.FILE, null, statCache, false)
    }
  ...
  test("test addResource appmaster only") {
    ...
    val statCache: Map[URI, FileStatus] = HashMap[URI, FileStatus]()
    val realFileStatus =
      new FileStatus(20, false, 1, 1024, 10, 30, null, "testOwner", null, new Path("/tmp/testing"))
    when(fs.getFileStatus(destPath)).thenReturn(realFileStatus)

    distMgr.addResource(
        fs, conf, destPath, localResources, LocalResourceType.ARCHIVE, "link", statCache, true)
    val resource = localResources("link")
    assert(resource.getVisibility() === LocalResourceVisibility.PRIVATE)
    assert(ConverterUtils.getPathFromYarnURL(resource.getResource()) === destPath)
  ...
  test("test addResource archive") {
    ...
    distMgr.addResource(
        fs, conf, destPath, localResources, LocalResourceType.ARCHIVE, "link", statCache, false)
    val resource = localResources("link")
    assert(resource.getVisibility() === LocalResourceVisibility.PRIVATE)
    assert(ConverterUtils.getPathFromYarnURL(resource.getResource()) === destPath)
    assert(resource.getTimestamp() === 10)
